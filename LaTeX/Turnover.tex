...
Overall, our models reveal that transitions between sex-determination systems, particularly transitions where the heterogametic sex changes, can be driven by loci in unexpected genomic locations that experience selection during diploid and/or haploid phases, which might be reflected by the lability with which sex-determination systems evolve. %and for the factors determining the phylogenetic pattern of sex-determination systems. 
\end{abstract}
...
Another significant feature of our model is that we consider selection loci that are under selection and also in very tight linkage with the ancestral sex-determining region.
...
Indeed, Figure \ref{fig:regionplots}A indicates that $\lambda_{mA}$ tends to be larger than one with sexually-antagonistic selection where the $a$ allele is strongly favoured in males ($w_{aa}$ much larger than $w_{Aa}$).
In this case, W-$A$ haplotypes will only create females with high fitness ($AA$ or $Aa$ genotypes) and therefore have higher fitness than ancestral females, which sometimes also produce $aa$ females. 
...
Both W-$A$ and W-$a$ haplotypes can spread, as is the case when $AA$ individuals have low fitness in females yet the $A$ is polymorphic or fixed on the X due to overdominance in males (Figure \ref{fig:regionplots}B and \ref{fig:regionplots}C).
...
We use this example to discuss why heterogametic transitions can occur when $R=1/2$ and $r<1/2$, as in Table \ref{tab:specialcases}.
...
Thus, a key factor in explaining why heterogametic transitions can occur when $R>r$ is that that the neo-SDR determines sex in the diploid phase but then recombination occurs before any subsequent haploid selection. 
...
For instance, alleles favoured by haploid selection in males often become associated with the Y, which leads to a male sex-ratio bias.
...
We show that the spread of new sex determination systems can be driven by loci experiencing haploid selection. 
...
\textcolor{red}{
Check that we mention Sally's result that invasion cannot occur with sexually-antagonistic selection and $R=1/2$
}

\textcolor{red}{
We could also give versions of Figure \ref{fig:regionplots} where there is also haploid selection of various types. I suggest using $\alpha_{\Delta}^\Hermaphrodite=1/20$ and $t^\Hermaphrodite=1/10$. 
Haploid selection can favour $A$ or $a$, so this would involve 4x 6-panel figures. Started looking at this in RegionPlots.nb but haven't added gametic competition or labels for $\lambda$s. 
Try to integrate into the discussion of haploid selection?}
...
We could also show an example where XY and ZW sex determining systems are both polymorphic and stable.  
}
