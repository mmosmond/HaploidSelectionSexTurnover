...
Neo-W alleles, on the other hand, can invade an ancestral XY system under some conditions (the full invasion conditions are given in the appendix). 
Significantly, we note that it is possible for both neo-W haplotypes to spread ($\lambda_{mA}>1$ and $\lambda_{ma}>1$ in \ref{Binvasion} and \ref{Ainvasion}), in which case neo-W invasion can occur regardless of linkage to the selected locus.
That is, selection on loci within the non-recombining region of the SDR can favour an unlinked neo-W. 
...
\textcolor{blue}{
Given that the $a$ allele can be considered ancestrally `male-beneficial' because it is fixed on the Y, it might be surprising that neo-W$a$ haplotypes can be favoured by selection in females, i.e., $\lambda_{ma}>1$. 
Again, this occurs because ancestral X's sometimes experience selection in males, in which they will always be paired with a Y$a$. 
...
Therefore, the X can be polymorphic or fixed for the $A$ allele despite favouring the $a$ allele during selection in females \textcolor{red}{(see also Lloyd 197?, Otto 2014)}. 
In such cases, neo-W$a$ haplotypes can spread. 
...
\textcolor{blue}{
We plot parameters under which $\lambda_{mA}>1$ and $\lambda_{ma}>1$ in figure XXX with diploid selection only. 
Wherever both haplotypes have positive growth rates, invasion by a neo-W is expected regardless of its linkage with the selected locus (i.e., even unlinked neo-W alleles can invade). 
In regions where at least one of these haplotypic growth rates is positive, a new neo-W feminizing mutation as long as \eqref{eq:lambdasGen} is satisfied, which can require that the recombination rate, $R$, is low enough and yet still indicates that more loosely linked sex-determining regions can spread. 
}
