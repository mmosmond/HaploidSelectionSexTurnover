...
%and $g_j^*$ is the growth rate of $Xm$ and $Ym$ haplotypes when on an $A$ ($j=A$)  or $a$ background ($j=a$).
...
%Assuming $g_A^* > 0$ and $g_a^* < 0$ without loss of generality, this is $-(1 - p_{Xf}) w_{m,A} g_a^* < p_{Xf} w_{m,a} g_A^*$.
...
\begin{equation}
\lambda_{Y,XY} \approx 1 + V_A \frac{(r - R)s_f}{r R} \frac{s_f}{\left( s_f + s_m \right)^2} t_m^2,
\end{equation}
where $V_A = p_A(1-p_A)$ is the variance at the \textbf{A} locus, $s_f$ and $s_m$ are the respective selection coefficients for $A$ in diploid females and males, $t_m$ is the selection coefficient for $A$ in gametes/gametophytes from males, and we've assumed equal dominance coefficients in the two sexes.
...
\begin{equation}
\lambda_{W,XY} \approx 1 + V_A \frac{(2r(1-R)-R) s_f + (1-2r)R s_m }{2 r R} \frac{s_f}{ \left( s_f + s_m \right)^2} t_m^2,
\end{equation}
...
\begin{equation}
B = \frac{s_f}{2r R(s_{f}+s_{m})^2} t_m^2
\end{equation}
and $C_k$ is a term that depends on $k$:
...
%k=1
\begin{equation}
C_1 = - \left[ (2r(1-R)-R) s_f + (1-2r)R s_m \right]
\end{equation}
...
In the case of $k=1$ the maternal and offspring controlled results are the same (which makes sense because we are here concerned only with the passage of mutations through females).	
