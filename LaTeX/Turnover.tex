...
Therefore, the invasion of rare mutant neo-Y or neo-W alleles can be simplified and given by the largest eigenvalue that solves the quadratic characteristic polynomial
...
\caption{Parameters determining invasion for neo-Y or neo-W alleles}
\begin{tabular}{l}
\hline\hline
   \noalign{\vskip 0.5ex}
  ...
  $\rho_{mA}=R (1-p_{X}^{\female}) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1-\alpha^{\male})/ \{ \bar{w}_{H}^{\female} \bar{w}_{H}^{\male} \bar{w}^\male \} $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{ma}=R p_{X}^{\female} w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male} \alpha^{\male}/ \{ \bar{w}_{H}^{\female} \bar{w}_{H}^{\male} \bar{w}^\male \}$\\ [1ex] \hline 
  \noalign{\vskip 0.5ex}
  ...
  $\rho_{mA}=R (1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female} (1-\alpha^{\female})/ \{\bar{w}_{H}^{\female} \bar{w}_{H}^{\male} \bar{w}^\female \} $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{ma}=R\bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female} \alpha^{\female}/ \{\bar{w}_{H}^{\female} \bar{w}_{H}^{\male} \bar{w}^\female \}$\\ [1ex] 
  \hline \hline 
   \end{tabular}
      \begin{tablenotes}
      \scriptsize
      \item $\bar{p}^{\male}=p_{Y}^{\male}q+p_{X}^{\male}(1-q)$ is the average frequency of the $A$ allele among X- and Y-bearing male gametes 
      \item see Table \ref{tab:meanfitnesses} for mean fitnesses.
...
%  $r<1/2$, $R=1/2$ &  \\ [0.5ex] \hline
  ...
  $h^\male=h^\female$, $t^\female=\alpha^\female_{\Delta}=\alpha^\male_{\Delta}=0$ & $s^\female(s^\male-s^\female)>0$ \\
  $h^\male=h^\female$, $t^\male=\alpha^\female_{\Delta}=\alpha^\male_{\Delta}=0$ & $s^\male(s^\female-s^\male)>0$ \\
  \hline \hline
...
If so, then we should see sex ratio influence the invasion of a novel sex-determining region that causes half of its carries to become female and half to become male.
However, we find that the growth rate of a rare, dominant offspring-controlled neo-ESD region that produces males or females with equal probability ($k=1/2$) is
\begin{equation}
\lambda_{ESD} \approx 1 + \frac{1}{2}\frac{(\lambda_{Y,XY} - 1) + (\lambda_{W,XY} -1)}{2} \Big|_{R=1/2},
...
Thus by the same argument as above (if drive in males only then sex ratio only biased when ancestrally XY), sex ratio selection does not drive the turnover to a perfect ($k=1/2$), offspring-controlled neo-ESD.
