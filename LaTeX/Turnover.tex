...
%\textcolor{blue}{
In Figure \ref{fig:regionplot_ 2a} we show a region of parameter space under which both neo-W haplotypes invade ($\lambda_{mA}>1$ and $\lambda_{ma}>1$) when there is overdominance in females and no haploid selection \citep[corresponding to Figure 2a in][]{Otto2014}. 
...
We might therefore predict a difference when we  XY to ZW transitions 
However, under weak selection we find that invasion by a homogametic neo-sex-determining allele (XY to XY or ZW to ZW) or by a heterogametic neo-sex-determining allele (XY to ZW or ZW to XY) occur under the same conditions. 
This is because, with weak selection, male biased zygotic sex ratios ($\zeta-1/2$) favouring the invasion of a neo-W are of the same order as meiotic drive favouring the invasion of a driving ne $\epsilon^2$. 
...
\textcolor{blue}{The characteristic polynomial describing eigenvalues of the 8 equation system (describing growth rate on XA, Xa, YA, Ya backgrounds in males and females) does not reduce for ESD mutants as it does for $k=0$ or $k=1$. Therefore we focus on weak selection here.}
...
\end{tikzpicture}
\caption{
Gray region depicts the relative fitnesses of male homozygotes, $w_AA^\male$ and $w_aa^\male$, that allow both neo-W haplotypes to invade, from any stable resident equilibria (equations \ref{eq:tightequil}).
In the white region neo-W haplotypes paired with the allele fixed on the Y cannot invade.
Parameters as in \cite{Otto2014} Figure 2a: $w_A^\female=w_a^\female=w_A^\male=w_a^\male=1$, $\alpha^\female=\alpha^\male=1/2$, $w_Aa^\female=w_Aa^\male=1$, and $w_AA^\female=w_aa^\female=0.75$, with $r=0$.
}
\label{fig:regionplot_ 2a}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
