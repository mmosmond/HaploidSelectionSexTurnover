...
\textcolor{red}{Switch between $\chi$ and $\rho$ in all places because $\chi$ is used for double recombination events.}
\textcolor{blue}{I think we need a different variable given the $\rho$ terms in the characteristic polynomial. Also, where is $\chi$ used for double recombination? In other papers? Here $\chi$ is the probability of an odd number of cross-overs between the SDR and M loci.}
...
The $\lambda_{mi}$ and $\rho_{mi}$, and thus the spread of the mutant $m$ allele, depend on the frequency of alleles at the \textbf{A} and \textbf{X} loci in the ancestral population. 
...
\hline\hline
  ...
  $\lambda_{mA} = \left( 2 \zeta \right)^{-1} \left[p_{X}^{\female} w_{A}^{\female} w_{A}^{\male} w_{AA}^{\male} + (1-p_{X}^{\female}) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right) $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\lambda_{ma} = \left( 2 \zeta \right)^{-1} \left[(1-p_{X}^{\female}) w_{a}^{\female} w_{a}^{\male} w_{aa}^{\male} + p_{X}^{\female} w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male}(1 - \alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right) $ \\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{mA} = R \left( 2 \zeta \right)^{-1} \left[ (1-p_{X}^{\female}) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/  \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right)   $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{ma} = R \left( 2 \zeta \right)^{-1} \left[   p_{X}^{\female} w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male} (1 - \alpha_\Delta^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right)  $\\ [1ex] \hline 
  \noalign{\vskip 0.5ex}
  ...
  $\rho_{mA} = R \left[ 2 (1 - \zeta) \right]^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female} (1+\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^\female \right)  $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{ma} = R \left[ 2 (1 - \zeta) \right]^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female} (1-\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^\female \right) $\\ [1ex] 
  \hline \hline 
   \end{tabular}
      \begin{tablenotes}
      \scriptsize
      \item $\bar{p}^{\male}=(1-q)p_{X}^{\male} + qp_{Y}^{\male}$ is the average frequency of the $A$ allele among X- and Y-bearing male gametes.
...
In order to explicitly determine the conditions under which a rare neo-sex-determining allele spreads, we must calculate the equilibrium frequency of the $A$ allele in the ancestral population (i.e., $\hat{p}^\female_X$, $\hat{p}^\male_X$, and $\hat{p}^\male_Y$). 
...
\textcolor{how will we differentiate between equilibrium $p$ and non-equilibrium $p$? or is the point that we can only say anything when at equilibrium?}
...
Assuming that selection is weak relative to all recombination rates ($r$, $R$ and $\chi$), we denote the leading eigenvalues describing the invasion of a neo-Y ($k=0$) and a neo-W ($k=1$) into an ancestrally XY system by $\lambda_{Y',XY}$ and $\lambda_{W',XY}$, respectively. To leading order in selection, these are:
...
In this figure, selection is ploidally antagonistic with haploid selection favouring the $a$ allele during male meiosis ($s^\female =s^\male = 0.2$, $h^\female = h^\male = 0.7$, $t^\female = t^\male = \alpha^\male_\Delta = 0$).
...
Parameters:  $s^\female =s^\male = 0.2$, $h^\female = h^\male = 0.7$, $t^\female = t^\male = \alpha^\male_\Delta = 0$, $\alpha^\male_\Delta = -0.2$, $r=0.02$.
}
...
Firstly, we show that selection (including sexually-antagonistic selection) on loci within or near the non-recombining region of the ancestral sex-determining region can favour heterogametic transitions (XY to ZW or ZW to XY) to new sex-determining systems that have looser linkage (e.g., see Figure \ref{fig:SexAntagTighter}). 
Secondly, assuming that selection is weak relative to recombination, we show that new sex-determining alleles are typically favoured if they are more closely linked to a locus under haploid selection, which is the only condition favouring homogametic transitions (XY to XY or ZW to ZW). 
...
and via fitness effects of alleles that are associated with the neo-sex-determining allele, and these selection pressures are predicted to often be of roughly equal magnitude (unless linkage is tight). 
...
We therefore argue that haploid selection should be considered as a pivotal factor in the evolution of sex-determination systems. 
...
Table \ref{tab:chisubstitutions} shows the value of $\chi$ in the absence of cross-over interference for each possible ordering of the loci.
...
These four variables determine the frequencies of the six resident gamete types: $x_{1}^{\female}=p_{X}^{\female}$, $x_{2}^{\female}=1-p_{X}^{\female}$, $x_{1}^{\male}=(1-q)p_{X}^{\male}$, $x_{2}^{\male}=(1-q)(1-p_{X}^{\male})$, $y_{1}^{\male}=q p_{Y}^{\male}$, and $y_{2}^{\male}=q(1-p_{Y}^{\male})$. 
...
For the $a$ allele to be stably fixed on the Y we need $\bar{w}_{Ya}^{\male} > \bar{w}_{YA}^{\male}$ where $\bar{w}_{Ya}^{\male} = w_{a}^{\male} \big[p_{X}^{\female}(1-\alpha^\male_\Delta) w_{A}^{\female} w_{Aa}^{\male} + (1-p_{X}^{\female})w_{a}^{\female} w_{aa}^{\male} \big]$ and $\bar{w}_{YA}^{\male} = w_{A}^{\male} \big[ p_{X}^{\female} w_{A}^{\female} w_{AA}^{\male} + (1-p_{X}^{\female})(1+\alpha^\male_\Delta) w_{a}^{\female} w_{Aa}^{\male} \big]$. 
...
Fixation of the $a$ allele on the X (equilibrium $B$) can be stable only if equilibrium $(A)$ is not and requires $\psi<0$ and $w_{A}^{\female}w_{AA}^{\female}>(1-\alpha^{\female})w_{a}^{\female}w_{Aa}^{\female}$. 
...
{ 2w_{AA}^\female } \\
\rho_{mA}&= \frac{1}{2} \left[w_{a}^\male (1-\alpha^\male_\Delta) \right]^{-1}
\frac{w_{A}^\female }{w_{A}^\female}
...
Meiotic drive in females will also change the proportion of gametes that carry the $A$ versus $a$ alleles, which will be produced by heterozygous females in proportions $(1+\alpha_{\Delta}^\female)$ and $(1-\alpha_{\Delta}^\female)$, respectively. 
...
For this equilibrium to be stable, X-$A$ must be sufficiently favoured in males (for example, by overdominance in males, remembering that $a$ is fixed on the Y) to keep $A$ fixed on the X. 
Specifically, from the stability conditions for equilibrium (B), we must have $w_{Aa}^\male/\big[(w_{aa}^\male+w_{Aa}^\male)/2\big]>w_{Aa}^\female/w_{AA}^\female$.

Still considering $w_{Aa}^\female>w_{AA}^\female$, the neo-W can also spread alongside the $a$ allele ($\lambda_{ma}>1$) if there is sufficiently strong underdominance in females ($w_{aa}^\female>w_{Aa}^\female$) \textcolor{red}{[this is describing directional selection, not overdominance - check the conditions for the case we want to talk about]}, such that $(w_{Aa}^\female+w_{aa}^\female)/2>w_{AA}^\female$.  
...
\rho_{mA}&= \frac{a}{b} \frac{R}{2} \left[   w_{Aa}^\female (1+\alpha^\female_\Delta) w_a^\male \frac{c}{d} \right] / w_a^\female\\ 
\rho_{ma}&= \frac{a}{b} \frac{R}{2} \left[ w_{Aa}^\female (1-\alpha^\female_\Delta) w_{Aa}^\male w_A^\male (1+\alpha^\male_\Delta) \phi \right] / w_A^\female
\end{align}
\end{subequations}
...
c &= w_{Aa}^\male (1-\alpha^\male_\Delta) \phi \left[ w_{Aa}^\male (1+\alpha^\male_\Delta) \phi \right] + 2w_{Aa}^\male\phi(w_{AA}^\male \psi + w_{aa}^\male \alpha^\male \psi ) + 2w_{aa}^\male \psi w_{AA}^\male \psi \\
d &= w_{Aa}^\male (1+\alpha^\male) \phi + w_{aa}^\male \psi
\end{align}
\end{subequations}
...
Add asterisk to Figure \ref{fig:regionMaleDrive}B to show parameters used in this plot. 
}
$w_{aa}^\female=1.05$, $w_{Aa}^\Hermaphrodite=1$, $w_{AA}^\female=0.85$, $w_{aa}^\male=0.85$, $w_{AA}^\female=1.05$, $\alpha_{\Delta}^\male=-0.08$.
}
...
DEF, $\alpha_{\Delta}^\male=0.08$. \textcolor{red}{Panel F mislabelled, should have $\lambda_{Wa}>1$ \& $\lambda_{WA}<1$ as the upper label that has the line.}
}
