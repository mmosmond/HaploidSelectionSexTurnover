...
We use the term `meiotic drive' to refer to the biased (non-Mendelian) segregation of genotypes during gamete production and the term `gametic competition' to refer to selection upon haploid genotypes within a gamete/gametophyte pool; the term `haploid selection' encompasses both processes. 
...
\textcolor{red}{check these interpretations (note the 2s in the lambdas of table 2). the $\lambda$s are certainly related to true growth rates because we get them by looking at the case where $R=0$ so that you only have the haplotypes growing/declining. }
...
Therefore, a novel environmental sex-determiner that causes half of its carriers to become female and half to become male ($k=1/2$) will be in males half of the time and in females half of the time (like an autosome), and so should be favoured by sex ratio selection.
...
This prediction could be examined using a suitable proxy for haploid selection, for example, \cite{Lenormand:2005vb} use the outcrossing rate as a proxy for the strength of pollen competition. 
