...
\hline\hline
  ...
  $\lambda_{mA} = \left\{ 2 (1- \zeta) \right\}^{-1} \left[p_{X}^{\female} w_{A}^{\female} w_{A}^{\male} w_{AA}^{\male} + (1-p_{X}^{\female}) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right) $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\lambda_{ma} = \left\{ 2 (1- \zeta) \right\}^{-1} \left[(1-p_{X}^{\female}) w_{a}^{\female} w_{a}^{\male} w_{aa}^{\male} + p_{X}^{\female} w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male}(1 - \alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right) $ \\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{mA} = R \left\{ 2 (1- \zeta) \right\}^{-1} \left[ (1-p_{X}^{\female}) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/  \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right)   $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{ma} = R \left\{ 2 (1- \zeta) \right\}^{-1} \left[   p_{X}^{\female} w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male} (1 - \alpha_\Delta^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^\male \right)  $\\ [1ex] \hline 
  \noalign{\vskip 0.5ex}
  neo-W ($k=1$) \\ [0.5ex] \hline \noalign{\vskip 1ex}
  $\lambda_{mA} = \left( 2 \zeta \right)^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{A}^{\female} w_{AA}^{\female}+(1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female}(1+\alpha_{\Delta}^{\female})\right]/ \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^\female \right) $ \\ [0.5ex] \noalign{\vskip 0.5ex}
  $\lambda_{ma} = \left( 2 \zeta \right)^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{a}^{\female} w_{aa}^{\female}+\bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female}(1-\alpha_{\Delta}^{\female})\right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^\female \right) $ \\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{mA} = R \left( 2 \zeta \right)^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female} (1+\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^\female \right)  $\\ [0.5ex] \noalign{\vskip 0.5ex}
  $\rho_{ma} = R \left( 2 \zeta \right)^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female} (1-\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^\female \right) $\\ [1ex] 
  \hline \hline 
   \end{tabular}
      \begin{tablenotes}
      \scriptsize
...
First, Fisherian sex-ratio selection will favour the spread of a neo-Y if the ancestral zygotic sex ratio is biased towards females, $\zeta>1/2$ (i.e., $\zeta>1/2$ causes the first factor of the $\lambda_{mi}$ to be greater than one for a neo-Y and vice versa for a neo-W).
However, the spread of a neo-Y (neo-W) also depends on the male (female) fitness of associated alleles (terms in square brackets). %, see terms in curly brackets. 
...
\begin{align}
\lambda_{mA}&= (2 \alpha^\male w_{A}^\male)^{-1}
\frac{w_{A}^\female }{w_{A}^\female}
\frac{\left[w_{A}^\male \alpha^\male w_{AA}^\female + 
w_{a}^\male (1-\alpha^\male) w_{Aa}^\female 2\alpha^\female\right]}
{ w_{AA}^\female } \\
\lambda_{ma}&= (2 \alpha^\male w_{A}^\male)^{-1} 
\frac{w_{a}^\female}{w_{A}^\female}
\frac{\left[ w_{A}^\male \alpha^\male w_{Aa}^\female2(1-\alpha^\female)+
w_{a}^\male (1-\alpha^\male) w_{aa}^\female \right]}
{ w_{AA}^\female } \\
\rho_{mA}&= (2 \alpha^\male w_{A}^\male)^{-1}
\frac{w_{A}^\female }{w_{A}^\female}
\frac{R w_{a}^\male (1-\alpha^\male) w_{Aa}^\female  \alpha^\female}
{ w_{AA}^\female } \\
\rho_{ma}&= (2 \alpha^\male w_{A}^\male)^{-1}
\frac{w_{a}^\female}{w_{A}^\female}
\frac{R w_{A}^\male \alpha^\male w_{Aa}^\female (1-\alpha^\female)}
{  w_{AA}^\female} 
\end{align}
\end{subequations}
...
Haploid selection impacts the spread of neo-W haplotypes in three ways.
Firstly, the zygotic sex ratio becomes male biased, $\zeta<1/2$, when the $a$ allele (which is fixed on the Y) is favoured during competition among male gametes or by meiotic drive in males.
Specifically, at equilibrium $(B)$, the sex ratio is $\zeta = \alpha^\male w_A^\male / \bar{w}_{H}^\male$ where $\bar{w}_{H}^\male=[(1-\alpha^\male)w_a^\male + \alpha^\male w_A^\male]$ has been canceled in equations \eqref{Binvasion} to leave the term $(2 \alpha^\male w_{A}^\male)^{-1}$. 
...
At equilibrium $(B)$, the fitness of female gametes under the ancestral sex-determining system is $w_{A}^\female$ such that the relative fitnesses of neo-W-$A$ and neo-W-$a$ haplotypes during female gametic competition are $w_{A}^\female/w_{A}^\female$ and $w_{a}^\female/w_{A}^\female$, see terms in equation \eqref{Binvasion}. 
...
At equlibrium $(B)$, neo-W female gametes will mate with X-$A$ male gametes with probability $\alpha^\male w_A^\male / \bar{w}_{H}^\male$ and Y-$a$ male gametes with probability $(1-\alpha^\male) w_a^\male / \bar{w}_{H}^\male$, where the $\bar{w}_{H}^\male$ terms have been canceled in \eqref{Binvasion}. 
Thus, for example, neo-W-$A$ haplotypes are found in $AA$ female diploids with probability $\alpha^\male w_A^\male / \bar{w}_{H}^\male$ (first term in square brackets in equation \ref{Binvasion}a) and in $Aa$ female diploids with probability $(1-\alpha^\male) w_a^\male / \bar{w}_{H}^\male$ (see equation \ref{Binvasion}c and second term in square brackets in equation \ref{Binvasion}a).
