...
In this case $b= - (\lambda_{mA} + \lambda_{ma})$ and $c = \lambda_{mA} \lambda_{ma} -\rho_{mA} \rho_{ma}$, where $2\lambda_{mi}-1$ is the growth rate of mutant haplotypes on background $i\in\{A,a\}$, accounting for loss due to recombination in heterozygotes, and $\rho_{mi}-1$ is the rate of addition of mutant haplotypes onto background $i\in\{A,a\}$ due to recombination in heterozygotes (see table \ref{tab:haplotype_growth}).
\textcolor{red}{check these interpretations as there are weird 2s in table 2}

\begin{threeparttable}[ht]
\centering
\smallskip
%\caption{Parameters determining invasion (equation \ref{eq:charpoly_neoY}) for neo-Y or neo-W alleles}
\caption{Parameters determining invasion of neo-Y or neo-W alleles into ancestral XY system}
\begin{tabular}{l}
\hline\hline
   \noalign{\vskip 0.5ex}
