...
We assume that all gametes compete for fertilization during haploid competition, which is not the case for monogamous mating systems where gametes from only one mating partner are present. 
Haploid competition in monogamous mating systems is equivalent to meiotic drive in our model, which only alters the frequency of gametes produced by heterozygotes. 
...
\hline\hline
  ...
  \hline \hline 
   \end{tabular}
      ...
      \item $\bar{w}_H^\female = p_X^\female w_A^\female + (1-p_X^\female) w_a^\female$ is the mean fitness of female gametes during haploid competition
      \item $\bar{w}_H^\male = \bar{p}^{\male} w_A^\male + (1-\bar{p}^{\male}) w_a^\male$ is the mean fitness of male gametes during haploid competition
...
We only consider selection at the \textbf{A} locus, i.e., sex-determining alleles are not directly selected except via their associations with sex and \textbf{A} locus alleles. 
...
During heterogametic transitions (XY to ZW or ZW to XY), the formally sex-limited allele fixes (Figure \ref{fig:Combination_Turnover}) such that all individuals have YY or WW genotypes. 
...
However, many meiotic drive systems involve an interaction with another locus at which alleles may `suppress' the action of meiotic drive. 
...
Note that, ploidally-antagonistic selection does not suffer from the proviso that it is easier to maintain polymorphisms close to the SDR.  vD\&K also suggest that this build up of sex-antagonistic polymorphisms may help to stabilize the ancestral sex-determining system, which would not be the case with haploid selection. 
