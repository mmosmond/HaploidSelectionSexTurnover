...
For a neo-Y, invasion depends on the relative lifetime fitness of $A$-bearing and $a$-bearing male gametes (i.e., in sperm only). 
...
Similarly, invasion of a neo-W depends on the relative lifetime fitness of $A$-bearing and $a$-bearing female gametes (i.e., in eggs only). 
...
\frac{(1 - p^\female_X) w^\female_a w^\male_A \alpha^\male}{ \bar{w}^\female_H \bar{w}^\male_H (\lambda_{ma}-1)} \right] 
...
\frac{(1 - \bar{p}^\male) w^\male_a w^\female_A \alpha^\female}{ \bar{w}^\male_H \bar{w}^\female_H (\lambda_{ma}-1)} \right] 
\frac{w^\male_{Aa}}{\bar{w}^\male} < 1,
\end{equation}

\noindent
\textcolor{red}{FIX THESE EQUATIONS... i changed them a little, is this okay now? (mmo)}
