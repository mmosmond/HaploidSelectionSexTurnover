...
\hline\hline
  ...
  A & $\alpha^d=1/2+\alpha_{\Delta}^{d}$ \\
  a & $(1-\alpha^d)=1/2-\alpha_{\Delta}^{d}$ \\
  \hline \hline
...
The frequency of Y among male gametes is proportional to the difference in $A$ allele frequency on X- and Y-bearing male gametes, $q=1/2+\alpha_{\Delta}^m(\hat{p}^m_Y-\hat{p}^m_X)+O(\epsilon^3)$.
...
Thus, eggs with and without a neo-W differ in the frequency of alleles they obtain from mating with male gametes. 
...
That is, $\lambda_{XY,XY}=\lambda_{ZW,ZW}$ and $\lambda_{XY,ZW}=\lambda_{ZW,XY}$, where $\lambda_{i,j}$ indicates the growth rate of a new mutant giving sex-determining system $j$ in a population in which sex is ancestrally determined by system $i$. 
