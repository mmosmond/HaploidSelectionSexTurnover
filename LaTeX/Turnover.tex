...
Given the characteristic polynomial and the Perron-Forbenius theorem, at least one $\lambda$ is greater than one when $b<2$ (see supplementary material for details), or  $(\lambda_{mA} - \chi_{mA})/2 + (\lambda_{ma} -  \chi_{ma})/2 > 1$.
In words, the neo-sex-determining allele will certainly spread when the average change in its frequency, considering that recombination breaks up mutant haplotypes but ignoring the fact that recombination also creates mutant haplotypes, is positive.
We are also guaranteed to have at least one $\lambda$ greater than one when 1 + b + c < 0.
This is never true if $\lambda_{mA} < 1$ and $\lambda_{ma} < 1$, i.e., if neither haplotype increases in frequency the neo-sex-determining allele will not invade. 
