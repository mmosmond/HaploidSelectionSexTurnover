...
The spread of a rare mutant $m$ at the \textbf{M} locus is determined by the leading eigenvalue, $\lambda$, of the system described by the equations for the next generation frequency of haploid genotypes with the mutation, (\ref{eq:recursions}c), (\ref{eq:recursions}d), (\ref{eq:recursions}g), (\ref{eq:recursions}h). %, evaluated at the above equilibrium.
%Here, we assume loci are arranged in the order \textbf{X}\textbf{A}\textbf{M} (i.e., $\chi=(1-r)R+(1-R)r$). 
%Invasion is numerically evaluated for different locus arrangements in figure \ref{fig:InvasionVsCentiMorgans}, with qualitatively similar results. %(i.e., $\chi = r R$). %(SAY SOMETHING HERE ABOUT OTHER ARRANGEMENTS?). %equal dominance coefficients in the two sexes,

\subsubsection*{Neo-$Y$}

Invasion of a rare, dominant neo-$Y$ ($k=0$) is determined by the largest eigenvalue that solves the characteristic polynomial
...
Table \ref{tab:chisubstitutions} gives substitutions for $\chi$ for defined relative locus locations.
