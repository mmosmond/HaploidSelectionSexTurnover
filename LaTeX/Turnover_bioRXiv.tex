...
\begin{figure}[!h]
\centering
\centerline{\includegraphics[width=1.25\linewidth]{../Plots/PositionPlot_SexAntagTighter}}
\caption{
...
\centerline{\includegraphics[width=1.25\linewidth]{../Plots/Region_plot_combined}}
\caption{
...
Figure \ref{fig:positionOverdominance} shows the dynamics arising with the parameters marked with a dagger. 
...
Wherever both haplotypic growth rates are greater than one, invasion by a neo-W is expected regardless of its linkage with the selected locus (i.e., for any $R$, see Figure \ref{fig:positionOverdominance} and Figure \ref{fig:temporalOverdominance} for examples). 
...
However, numerical simulations demonstrate that new sex-determining alleles do not necessarily reach pseudo-fixation, which can lead to the stable maintenance of a mixed sex-determining system, in which X, Y, Z, and W alleles all segregate (e.g., Figure \ref{fig:freqAll}B,C). 
...
In addition, we find that polymorphic sex determining systems (X, Y, W, and Z alleles all present) can be maintained when a selected locus is tightly linked to the ancestral sex-determining system (e.g., Figure \ref{fig:freqAll}B and Figure \ref{fig:freqAll}C), which is not possible with loose linkage~\cite{vanDoorn:2010hu}. 
...
In cases of ploidally-antagonistic selection, where there is a balance between alleles favored in the haploid stage and the diploid stage, neo-W alleles - even if unlinked to the selected locus - can spread (Figure \ref{fig:regionPloidAntag}).
...
Of particularly importance, when selection is sexually-antagonistic in diploids ($s^\female s^\male <0$ and $0<h^\Hermaphrodite<1$), an unlinked neo-W ($R=1/2$) cannot invade unless there is also haploid selection (see proof in \nameref{file:Mathematica}; Fig \ref{fig:SexAntagTighter} and Figure \ref{fig:SexAntagTighterMaleDrive}). 
More generally, haploid selection alters the conditions under which neo-W alleles can spread (compare Figure \ref{fig:regionMaleDrive}-Figure \ref{fig:regionFemaleGS} to Fig \ref{fig:regionplots}). 
...
Panels A-C in Figure \ref{fig:regionMaleDrive} and \ref{fig:regionMaleGS} show that neo-W haplotypes tend to spread for a wider range of parameters when sex ratios are male biased, compared to Fig \ref{fig:regionplots} without haploid selection. 
By contrast, male haploid selection in favour of the $A$ allele generates female-biased sex ratios and reduces $\Lambda_{W'A}^{(XY)}$ and $\Lambda_{W'a}^{(XY)}$, as demonstrated by panels D-F in Figure \ref{fig:regionMaleDrive} and Figure \ref{fig:regionMaleGS}. 

Female haploid selection generates direct selection on the neo-W-$A$ and neo-W-$a$ haplotypes as they spread in females. 
Thus, female haploid selection in favour of the $a$ allele tends to increase $\Lambda_{W'a}^{(XY)}$ and decrease $\Lambda_{W'A}^{(XY)}$, as shown by panels A-C in Figure \ref{fig:regionFemaleDrive} and Figure \ref{fig:regionFemaleGS}. 
Conversely, female haploid selection in favour of the $A$ allele increases $\Lambda_{W'A}^{(XY)}$ and decreases $\Lambda_{W'a}^{(XY)}$, see panels D-F in Figure \ref{fig:regionFemaleDrive} and Figure \ref{fig:regionFemaleGS}. 
...
For the parameters in B, neo-W alleles have negative invasion fitness when the Y-$A$ haplotype is ancestrally more common than Y-$a$ (compare blue to black curves in Figure \ref{fig:positionOverdominance}A and  Figure \ref{fig:positionOverdominance}B near the ancestral sex-determining locus). 
In contrast, the neo-W is not lost in panel D as it is favoured regardless of whether Y-$A$ or Y-$a$ haplotypes predominate (again, compare blue to black curves in Figure \ref{fig:positionOverdominance}C and  Figure \ref{fig:positionOverdominance}D). 
}
...
\begin{figure}[!h]
\centering
\centerline{\includegraphics[width=1.25\linewidth]{PositionPlot_SexAntagTighter_MaleDrive.eps}}
\caption{
...
\begin{figure}[!h]
\centering
\centerline{\includegraphics[width=1.25\linewidth]{Region_Plot_combined_MaleDrive.eps}}
\caption{
...
\begin{figure}[!h]
\centering
\centerline{\includegraphics[width=1.25\linewidth]{Region_Plot_combined_MaleGS.eps}}
\caption{
...
%Diploid selection parameters ($w_{ij}^\Hermaphrodite$) are the same as those in Figure \ref{fig:regionMaleDrive} and Figure \ref{fig:regionMaleGS}. 
...
\begin{figure}[!h]
\centering
\centerline{\includegraphics[width=1.25\linewidth]{Region_Plot_combined_FemaleGS.eps}}
\caption{
...
%Diploid selection parameters ($w_{ij}^\Hermaphrodite$) are the same as those in Figure \ref{fig:regionMaleDrive}, Figure \ref{fig:regionMaleGS}, and Figure \ref{fig:regionFemaleDrive}. 
...
\centering
\centerline{
\includegraphics[width=1.25\linewidth]{All_plot_combined_PloidAntag.eps}
}
\caption{
...
\centering
\centerline{
\includegraphics[width=1.25\linewidth]{Freq_plot_combined_PloidAntag.eps}
}
\caption{
