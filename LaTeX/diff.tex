...
%DIF DEL main.tex           Sat Feb 17 08:51:21 2018
%DIF ADD main_revised.tex   Thu Apr  5 18:16:47 2018
...
Variation at such loci can favour the spread of new sex-determining systems in which the heterogametic sex changes (XY to ZW or ZW to XY) and the new sex-determining region is less closely linked (or \DIFaddbegin \DIFadd{even }\DIFaddend unlinked) to the locus under selection, which is not \DIFdelbegin \DIFdel{predicted by }\DIFdelend \DIFaddbegin \DIFadd{expected from }\DIFaddend previous theory. 
(2) We \DIFdelbegin \DIFdel{also }\DIFdelend consider selection upon haploid genotypes either during gametic competition (e.g., pollen \DIFdelbegin \DIFdel{/sperm }\DIFdelend competition) or meiosis (i.e., non-Mendelian segregation), which can cause the zygotic sex ratio to become biased. 
...
With }\DIFaddend haploid selection, \DIFaddbegin \DIFadd{we find that }\DIFaddend transitions between male and female heterogamety can \DIFdelbegin \DIFdel{also }\DIFdelend evolve where linkage with the sex-determining locus is \DIFaddbegin \DIFadd{either strengthened or }\DIFaddend weakened. 
Furthermore, \DIFdelbegin \DIFdel{sex ratio biases may }\DIFdelend \DIFaddbegin \DIFadd{we find that the selective strengths exerted by the sex-ratio and associations with selected alleles are often equivalent. 
...
Our models \DIFdelbegin \DIFdel{extend these theories to }\DIFdelend include selection upon haploid genotypes (meiotic drive or gametic competition), which causes \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend biases and occurs differently in \DIFdelbegin \DIFdel{males and females}\DIFdelend \DIFaddbegin \DIFadd{male and female gametes}\DIFaddend . 
Surprisingly, we find \DIFdelbegin \DIFdel{that neither force (sex ratio selection nor }\DIFdelend \DIFaddbegin \DIFadd{the two forces (sex-ratio selection and }\DIFaddend associations with genes that have sex-specific effects) \DIFaddbegin \DIFadd{will often be equally strong, and thus neither }\DIFaddend dominates the spread of new sex-determining systems alone. 
Even more unexpectedly, we find that \DIFdelbegin \DIFdel{, to spread, }\DIFdelend new sex-determining alleles \DIFdelbegin \DIFdel{do not necessarily have to arise in closer linkage with genes that are differentially selected in males and females}\DIFdelend \DIFaddbegin \DIFadd{can spread despite being less closely linked to selected loci}\DIFaddend . 
...
%DIFDELCMD < %%%
\DIFdel{Predominant theories accounting for the spread of new }\DIFdelend \DIFaddbegin \DIFadd{In sum, accumulating evidence indicates that transitions between }\DIFaddend sex-determining \DIFdelbegin \DIFdel{systems by selection involve fitness differences between sexes (e.g., sexually antagonistic selection) or sex ratio selection~\mbox{%DIFAUXCMD
...
consider systems in which the ancestral sex chromosomes experience meiotic drive (e.
g., where driving X or Y chromosomes are inherited disproportionately often), which causes sex ratios to become biased~\mbox{%DIFAUXCMD
...
Linkage allows a stronger favourable association to build up between a male-beneficial allele, and a }\DIFaddend neo-Y \DIFaddbegin \DIFadd{allele, for example. 
Such associations can favour cis-GSD transitions~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu}}%DIFAUXCMD
, trans-GSD transitions~\mbox{%DIFAUXCMD
\cite{vanDoorn:2010hu}}%DIFAUXCMD
, and new partially-masculinizing }\DIFaddend or \DIFdelbegin \DIFdel{neo-W chromosomes) can then spread, which restore an even sex ratio. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Here we use mathematical models to find the conditions under which new }\DIFdelend \DIFaddbegin \DIFadd{partially-feminizing alleles in a population with ESD~\mbox{%DIFAUXCMD
\cite{Muralidhar2018}}%DIFAUXCMD
.
However, any sexually-antagonistic loci that are more closely linked to the ancestral }\DIFaddend sex-determining \DIFdelbegin \DIFdel{systems spread when individuals experience selection at both diploid and haploid stages, which allows fitness differences between the sexes and sex ratio biases to occur simultaneously. 
...
. 
We use the term `meiotic drive' to refer to the biased (non-Mendelian) segregation of genotypes during gamete production (from one parent) and the term `gametic competition' to refer to selection upon haploid genotypes within a gamete/gametophyte pool (potentially from multiple parents); the term `haploid selection' encompasses both processes. }\DIFdelend \DIFaddbegin \DIFadd{locus should develop similar, favourable associations and are expected to hinder the spread of a new sex-determining system~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu}}%DIFAUXCMD
. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Segregation distortion provides putative evidence of haploid selectionand can sometimes be attributed to meiotic drive }\DIFdelend %DIF > Tight linkage Model Feature and Result
...
Our analysis reveals that, under various forms of selection, the ancestral-X can carry suboptimal alleles for females }\DIFaddend and/or \DIFdelbegin \DIFdel{gametic competition~\mbox{%DIFAUXCMD
...
. 
Gametic competition isalso typically sex specific, occurring primarily among male gametes, because there are typically many more pollen/sperm than required for fertilization.
...
.
}\DIFdelend \DIFaddbegin \DIFadd{the ancestral-Y can carry female-beneficial alleles. 
This, in turn, can favour a new ZW sex-determining locus that has weaker linkage with loci under selection, which has not been apparent in previous theory \mbox{%DIFAUXCMD
\cite{vanDoorn:2010hu}}%DIFAUXCMD
. 
That is, we show that selected loci in very tight linkage with the ancestral GSD locus can favour trans-GSD transitions in which associations are actually weakened. 
}\DIFaddend 

\DIFdelbegin \DIFdel{There are various ways by which genes experiencing haploid selection could influence transitionsbetween }\DIFdelend %DIF > Haploid selection model feature and discussion of why needed
\DIFaddbegin \DIFadd{Most significantly, we include haploid selection (gametic competition or meiotic drive) in models describing cis-GSD, trans-GSD, and GSD to ESD transitions, which poses an apparent evolutionary problem. 
On one hand, haploid selection is typically sex-specific in that it usually occurs among gametes produced by one sex only~\mbox{%DIFAUXCMD
\cite{Mulcahy:1996ha,JOSEPH:2004haa,Ubeda:2005gw,Lindholm:2016cw}}%DIFAUXCMD
. 
Therefore, one might expect new }\DIFaddend sex-determining \DIFdelbegin \DIFdel{systems. 
If we assume that haploid selection at any particular locus predominantly occurs in one sex (e. 
g., meiotic drive during spermatogenesis), then such loci experience a form of sex-specific selection. 
In this respect, we might expect that haploid selection would affect transitions between sex-determining }\DIFdelend systems \DIFdelbegin \DIFdel{in a similar manner to sex-specific diploid selection (as explored in ~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu}}%DIFAUXCMD
). 
...
}\DIFdelend \DIFaddbegin \DIFadd{to benefit from close linkage with haploid selected loci, as found for sex-differences in diploid selection~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu, Muralidhar2018}}%DIFAUXCMD
. 
}\DIFaddend On the other hand, \DIFdelbegin \DIFdel{sex ratios can also become biased by linkage between the }\DIFdelend \DIFaddbegin \DIFadd{associations between }\DIFaddend sex-determining \DIFdelbegin \DIFdel{locus and a locus that harbours genetic variation in haploid fitness}\DIFdelend \DIFaddbegin \DIFadd{loci and haploid selected loci generate biased zygotic sex ratios, which should generally hinder the spread of new sex-determining systems. 
}
...
, considers ancestral-ESD (with no sex-ratio bias) and numerically show that new GSD alleles can spread if they arise in linkage with meiotic drive loci}\DIFaddend . 
...
. 
It is not immediately clear how the spread of newsex-determining systems would be influenced by the combination of sexratio biases and associations with haploid selected alleles}\DIFdelend \DIFaddbegin \DIFadd{a masculinizing allele spreads in association with an allele that is favoured during male meiosis, causing sex ratios to become male-biased and thereby suggesting that such beneficial associations overwhelm selection to balance the sex ratio. 
However, Kozielska et al. (2010)~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm} }%DIFAUXCMD
consider meiotic drive on an ancestral GSD sex chromosome (creating a sex ratio bias)}\DIFaddend . 
\DIFaddbegin \DIFadd{They find that new, completely unlinked, GSD systems can spread if they generate the rarer sex, creating a balanced sex ratio. 
...
Surprisingly, Fisherian sex ratio selectiondoes not dominate; it is possible for selection on linked alleles to drive turnover between sex-determining systemsdespite causing increasingly biased sex ratios. 
...
\DIFaddbegin \DIFadd{Here, we analytically find the conditions under which new GSD or ESD systems spread with generic linkage between the loci involved (and generic sex-specific haploid and diploid selection). 
Strikingly, we reconcile the results of Kozielska et al. (2010)~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm} }%DIFAUXCMD
and Ubeda et al. (2015)~\mbox{%DIFAUXCMD
\cite{Ubeda:2015fx} }%DIFAUXCMD
by showing that new GSD systems commonly spread when they increase }\textit{\DIFadd{or}} \DIFadd{decrease linkage with loci that experience haploid selection.
...
We find that loci tightly linked with the }\DIFdelend \DIFaddbegin \DIFadd{system are favoured }\textit{\DIFadd{equally strongly}} \DIFadd{in situations where sex-ratio biases increase or decrease (and in situations where sex-ratio biases are present or absent). 
Finally, we show that ESD may not evolve, even if the sex ratio is initially biased by haploid selection, which would not be predicted by previous theories for transitions to ESD~\mbox{%DIFAUXCMD
\cite{Charnov:1982wg,Bull:1983vi,West:2009we}}%DIFAUXCMD
. 
Together, our results suggest that sex ratio selection is not an overwhelming force driving transitions between }\DIFaddend sex-determining \DIFdelbegin \DIFdel{locus can drive transitions in which the heterogametic sex changes, even when the new sex-determining locus is less closely linked to loci under selection (either including haploid selectionor not ). 
...
Selection }\DIFdelend \DIFaddbegin \DIFadd{compete for fertilization, which implies a polygamous mating system. 
Relative fitnesses }\DIFaddend during gametic competition \DIFdelbegin \DIFdel{depends on the $\mathbf{A}$ locus genotype; relative fitnesses }\DIFdelend are given by \DIFdelbegin \DIFdel{$w_A^\Hermaphrodite$ and $w_a^\Hermaphrodite$ ($\Hermaphrodite \in \{\female,\male\}$}\DIFdelend \DIFaddbegin \DIFadd{$w_A^\circ$ and $w_a^\circ$ ($\circ \in \{\female,\male\}$}\DIFaddend ; see table \ref{tab:fitnesstable}).
...
\DIFaddbegin \DIFadd{\textcolor{red}{We do not consider fertility(?) or sexual selection.}
...
The leading eigenvalue for a rare neo-Y or neo-W allele, $m\in\{Y',W'\}$, is the largest value of \DIFdelbegin \DIFdel{X }\DIFdelend \DIFaddbegin \DIFadd{$x$ }\DIFaddend that solves $x^2+ b x + c = 0$.  %$(\lambda_m^{(i)})^2+ b \lambda_m^{(i)} + c =0$. 
The coefficients are $b= - (\Lambda_{mA}^{(XY)} + \Lambda_{ma}^{(XY)})+(\chi_{mA}^{(XY)} + \chi_{ma}^{(XY)})$ and $c = (\Lambda_{mA}^{(XY)} - \chi_{mA}^{(XY)}) (\Lambda_{ma}^{(XY)} - \chi_{ma}^{(XY)}) - \chi_{mA}^{(XY)} \chi_{ma}^{(XY)}$, where $\Lambda_{mi}^{(XY)}>0$ is the multiplicative growth rate (which we will call the ``haplotypic growth rate'') of the neo-sex determination allele $m$ on background $i\in\{A,a\}$ without accounting for loss due to recombination ($R=0$), and $\chi_{mi}^{(XY)}>0$ is the rate at which mutant haplotypes on background $i\in\{A,a\}$ recombine onto the other $\mathbf{A}$ locus background in heterozygotes (proportional to $R$\DIFaddbegin \DIFadd{; we call this the ``dissociative force" as it breaks down linkage disequilibrium}\DIFaddend ), see Table \ref{tab:haplotype_growth}.
...
\DIFdelbegin \DIFdel{Second, invasion by a neo-Y (neo-W) does not directly depend on the fitness of female (male) diploids. %DIF < (for a given set of equilibrium allele frequencies).
This is because a dominant neo-Y (neo-W) is always found in males (females), and therefore the frequency of the neo-Y (neo-W), $m$, only changes in males (females), Fig \ref{fig:model_outline}C,D. 
Finally, invasions by a neo-Y and }\DIFdelend \DIFaddbegin \DIFadd{This implies that neither sex-ratio selection or selection on associated alleles dominates the invasion dynamics of }\DIFaddend a \DIFdelbegin \DIFdel{neo-W }\DIFdelend \DIFaddbegin \DIFadd{new sex-determining allele, allowing the sex ratio to become more or less biased during a transition (as previously shown in two special cases; \mbox{%DIFAUXCMD
\cite{Kozielska:2010vm,Ubeda:2015fx}}%DIFAUXCMD
). 
%DIF > That is, 
%DIF > \vspace{0.5cm}
...
\textbf{\DIFadd{selection on loci within the non-recombining region around the sex-determining locus ($r\approx0$) allows the invasion of less closely linked sex-determining alleles ($r<R\leq1/2$) during a trans-GSD transition}} \DIFadd{(XY $\leftrightarrow$ ZW).
}\vspace{0.5cm}

\noindent \DIFadd{This does not depend on the }\DIFaddend form of selection maintaining a polymorphism (sexually-antagonistic selection, overdominance, ploidally-antagonistic selection, or some combination, Fig \ref{fig:SexAntagTighter}, \nameref{fig:temporalOverdominance}, \nameref{fig:regionPloidAntag}, and \nameref{fig:SexAntagTighterMaleDrive}).  
...
The invasion of completely unlinked neo-W alleles ($R = 1/2$) can occur with overdominance in males or with haploid selection \DIFaddbegin \DIFadd{(e.g., a case with male drive opposed by diploid selection in \mbox{%DIFAUXCMD
\cite{Kozielska:2010vm}}%DIFAUXCMD
...
\DIFdelbegin \DIFdel{To develop an intuition for how less closely linked neo-W alleles invade ($R>r$), we here }\DIFdelend \DIFaddbegin \DIFadd{Here, we }\DIFaddend focus on cases where there is no haploid selection and discuss the additional effect of haploid selection in \nameref{app:inv_cond}. 
...
Coloured markers show recombination rates for which the temporal dynamics of invasion are plotted in the inset, demonstrating that neo-W alleles can reach pseudo-fixation if they are more (black) or less (red) closely linked to a locus experiencing sexually-antagonistic selection \DIFaddbeginFL \DIFaddFL{(neo-W which are too loosely linked (blue and green) do not invade)}\DIFaddendFL . 
A very loosely linked neo-W does not spread in this case (blue and green lines overlap and go to 0 in inset). 
Fitness parameters are: $w_{AA}^\female = 1.05$, $w_{aa}^\male = 1.2$, $w_{aa}^\female = w_{AA}^\male = 0.85$, \DIFdelbeginFL \DIFdelFL{$w_{Aa}^\Hermaphrodite = 1$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$w_{Aa}^\circ = 1$}\DIFaddendFL ,  \DIFdelbeginFL \DIFdelFL{$t^\Hermaphrodite = \alpha^\Hermaphrodite_\Delta = 0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t^\circ = \alpha^\circ_\Delta = 0$}\DIFaddendFL .
...
%DIF > To develop an intuition for how less closely linked neo-W alleles invade ($R>r$), we here focus on cases where there is no haploid selection and discuss the additional effect of haploid selection in \nameref{app:inv_cond}. 
...
Here, there is no haploid selection \DIFdelbeginFL \DIFdelFL{$t^\Hermaphrodite = \alpha^\Hermaphrodite_\Delta = 0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t^\circ = \alpha^\circ_\Delta = 0$}\DIFaddendFL .
}
...
However, numerical simulations demonstrate that\DIFaddbegin \DIFadd{, with tight sex linkage, }\DIFaddend new sex-determining alleles do not necessarily reach pseudo-fixation, which can lead to the stable maintenance of a mixed sex-determining system, in which X, Y, Z, and W alleles all segregate (e.g., \nameref{fig:freqAll}B,C). 
...
However, with haploid selection and \DIFaddbegin \DIFadd{some ancestral sex-linkage (}\DIFaddend $r<1/2$\DIFaddbegin \DIFadd{; allowing allele frequency differences on the X and Y)}\DIFaddend , the additional term in Eq \eqref{eq:lambda_neoW} can be positive.
...
}\textbf{\DIFadd{new sex-determining alleles (causing cis- }\textit{\DIFadd{or}} \DIFadd{trans-GSD transitions) can spread if the arise in closer linkage with a locus that experiences haploid selection than the ancestral-sex-determining locus ($R<r$).}} 
\vspace{0.5cm}
...
}\textbf{\DIFadd{new sex-determining alleles (causing trans-GSD transitions) can spread if they arise in looser linkage with a locus that experiences haploid selection than the ancestral-sex-determining locus ($r<R$). }} 
\vspace{0.5cm}
...
\DIFaddFL{In panel A, male drive ($\alpha^\male_{\Delta} = -1/20$, $t^\circ = \alpha^\female_{\Delta} = 0$) opposes selection in diploids (no sex-differences: $s^\circ = 1/10$, $h^\circ = 7/10$), in which case the new sex-determining allele can invade regardless of its linkage with the selected locus ($R$).   
...
\DIFdelbegin \DIFdel{When there is no gametic competition and meiotic drive is in one sex only, an unlinked neo-W can invade as long as the same allele is favoured during diploid selection in males and females ($s^\female s^\male>0$, see Fig \ref{fig:Combination_Centimorgans}A and Fig \ref{fig:SexRatioBad}B). %DIF < , which is 50\% of the parameter space. 
When there is no meiotic drive and gametic competition occurs in one sex only, an unlinked neo-W can invade as long as the same allele is favoured in male and female diploid selection and there are sex differences in selection of one type (e.g., $s^\female(s^\male-s^\female)>0$, see Fig \ref{fig:Combination_Centimorgans}B). %DIF < , which is 25\% of the parameter space. 
...
\DIFaddbegin \DIFadd{We show that neo-W alleles can spread for when they are more closely }\textit{\DIFadd{or}} \DIFadd{more loosely linked to a locus that experiences haploid selection in Fig \ref{fig:Combination_Centimorgans} (c.f., Fig \ref{fig:SexAntagTighter}A for diploid sexually-antagonistic selection alone). 
In Fig \ref{fig:SexRatioBad}, we also show that trans-GSD transitions in sex determination during which linkage with a haploid selected locus increases }\textit{\DIFadd{or}} \DIFadd{decreases. 
When $R>r$, neo-W alleles can benefit from Y-associated alleles that have higher fitness in the female diploid phase but then dissociate from these alleles through recombination, which occurs before any subsequent haploid selection. 
}\DIFaddend 
...
\DIFdelbegin \DIFdel{Previous research suggests that when the ancestral sex-determining locus is linked to a locus that experiences haploid selection (e.g., meiotic drive), alleles at a new, unlinked sex-determining locus invade to restore equalsex ratios~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm}}%DIFAUXCMD
. 
...
\DIFaddbegin \DIFadd{Our model allows us to analytically compare GSD-GSD transitions where sex-ratio bias increases, decreases, or remains equal. 
...
\textbf{\DIFadd{when selection is weak relative to recombination, the presence or absence of sex-ratio biases do not create an assymetry in the likelihood of GSD-GSD transitions. }} 
\vspace{0.5cm}
...
\DIFaddbegin \DIFadd{In our deterministic model this means the fraction female is exactly $k$, even when $m$ is rare (exploring the effect of the variance induced by ESD would also be interesting).
...
Such correlations can favour environmental sex-determining systems by allowing each sex to be produced in the environment in which it has highest fitness; in the absence of these correlations, previous theory would predict that ESD is favoured when it produces more equal sex ratios than the ancestral system (see reviews by~\cite{Charnov:1982wg,Bull:1983vi,West:2009we}). %(e.g., maternal condition, mate quality, age, or host size)  
...
Thus, Fisherian \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend selection alone does not easily explain GSD to ESD transitions.
...
\DIFdelbegin \DIFdel{the evolution of ESD is not straightforwardly predicted by selection to balance the zygotic sex ratio when haploid selection is present. 
...
In contrast to these prevailing views}\DIFdelend \DIFaddbegin \section*{\DIFadd{Discussion}}

\DIFadd{New sex determination systems are typically expected to spread when they equalise the sex ratio and/or when they increase linkage with loci that experience sex-differences in selection~\mbox{%DIFAUXCMD
\cite{Blaser2012, vanDoorn2014re} }%DIFAUXCMD
(including sex-differences at the haploid stage, Conclusion 2a and~\mbox{%DIFAUXCMD
\cite{Ubeda:2015fx}}%DIFAUXCMD
). 
However}\DIFaddend , we show that \DIFdelbegin \DIFdel{selection (including sexually-antagonistic selection, overdominance, and/or ploidally-antagonistic selection) on loci tightly linked to the ancestral sex-determining locus can favour trans-GSD transitions (XY$\leftrightarrow$ ZW) to new sex-determining systems that are less closely linked to }\DIFdelend the \DIFdelbegin \DIFdel{selected loci (e.g., see Fig \ref{fig:SexAntagTighter}). 
Similarly, even when linkage is weak relative to selection, we show that %DIF < new sex-determining alleles are typically favoured if they are more closely linked to a locus under haploid selection (even in the absence of diploid sex differences), which is the only condition favouring cis-GSD transitions (XY to XY or ZW to ZW). 
%DIF < In addition, with haploid selection and weak selection, 
}\DIFdelend \DIFaddbegin \DIFadd{spread of new sex-determination systems is not dominated by selection to balance the sex ratio (Conclusions 3 \& 4, Fig \ref{fig:SexRatioBad}). 
Furthermore, we find that }\DIFaddend trans-GSD transitions can \DIFdelbegin \DIFdel{occur where the new }\DIFdelend \DIFaddbegin \DIFadd{be favoured in cases where linkage with the }\DIFaddend sex-determining locus is \DIFdelbegin \DIFdel{less closely linked to the locus under selection if }\DIFdelend \DIFaddbegin \DIFadd{actually weakened (Conclusions 1 \& 2b, Fig \ref{fig:SexAntagTighter}), especially when }\DIFaddend there is haploid selection (\DIFdelbegin \DIFdel{e.g.}\DIFdelend \DIFaddbegin \DIFadd{Conclusion 2b}\DIFaddend , Figs \ref{fig:Combination_Centimorgans} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{\& }\DIFaddend \ref{fig:SexRatioBad}). 



\DIFdelbegin \DIFdel{We find that the spread of new sex-determining systems cannot be simply predicted from their effect on the sex ratio. 
}\DIFdelend %DIF > We find that the spread of new sex-determining systems cannot be simply predicted from their effect on the sex ratio. 
On one hand, \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend biases caused by haploid selection can facilitate trans-GSD transitions or transitions from genetic to environmental sex determination. 
...
%DIF > What we would like to stress is that sex-ratio selection alone cannot predict when new sex-determining systems can evolve.
Significantly, with weak selection, we find that there is no difference in conditions allowing XY to ZW and ZW to XY transitions \DIFaddbegin \DIFadd{(Conclusion 3)}\DIFaddend , indicating that \DIFdelbegin \DIFdel{sex chromosome transitions }\DIFdelend \DIFaddbegin \DIFadd{transitions in sex determination }\DIFaddend are not predominantly predicted by their effect on the sex ratio (i.e., the \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend bias created by male haploid selection facilitates the spread of a neo-W allele into an XY system to the same degree that male haploid selection drives the spread of a neo-Y into a ZW system with a 1:1 sex ratio). 
...
%DIF > An asymmetry can develop when sex-linkage is tight (e.g., Fig \ref{fig:Combination_Centimorgans} near -25cM and 25cM) but under most circumstances we do not predict asymmetry between XY to ZW and ZW to XY transitions despite the presence/absence of sex-ratio selection. 
Thus, haploid selection can favour trans-GSD transitions both via \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend selection and via selection on alleles associated with the new sex-determining allele, and these selective pressures are often predicted to be of equal magnitude. 


%DIF < HAPLOID SELECTION (genomic location of haploid selected loci and comparative patterns) 
\DIFdelbegin \DIFdel{We have shown that the spread }\DIFdelend \DIFaddbegin \DIFadd{Our results indicate that haploid selection could be an important factor driving the evolution }\DIFaddend of new sex-determining systems\DIFdelbegin \DIFdel{can be driven by loci experiencing haploid selection . 
...
For example, recent studies have demonstrated that sperm competition in animals, even within a single ejaculate, can alter haploid allele frequencies and increase offspring fitness~\mbox{%DIFAUXCMD
\cite{Immler:2014im,Alavioon2017}}%DIFAUXCMD
.
Nevertheless, although precise estimates are uncertain, a relatively small proportion of all genes are thought to be expressed and selected during competition in animal sperm~\mbox{%DIFAUXCMD
\cite{Zheng:2001fi,JOSEPH:2004haa,Vibranovski:2010et}}%DIFAUXCMD
.  
In plants, however, haploid selection may be particularly important due to the presence of a multicellular haploid stage. 
...
%DIF > HAPLOID SELECTION (genomic location of haploid selected loci and comparative patterns) 
%DIF > We have shown that the spread of new sex-determining systems can be driven by loci experiencing haploid selection. 
\DIFaddend %Our results suggest that haploid selected loci that have been involved in driving turnovers between sex-determining systems could occur around the ancestral- or novel-sex-determining regions. 
...
\DIFaddend In general, we predict that haploid selection increases lability of sex-determining systems, particularly because haploid selection can cause transitions that increase or decrease sex-linkage (\DIFaddbegin \DIFadd{Conclusion 2B, }\DIFaddend e.g., the final state of the red line in Fig \ref{fig:SexRatioBad}B is the starting state in Fig \ref{fig:SexRatioBad}A). 
Frequent turnovers driven by haploid selection may help to explain the relative rarity of heteromorphic sex chromosomes in plants\DIFdelbegin \DIFdel{, which are thought to experience more selection during their multicellular haploid stage.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend If haploid selection is strong but selective differences between male and female diploids are weak, we \DIFdelbegin \DIFdel{find }\DIFdelend \DIFaddbegin \DIFadd{specifically predict }\DIFaddend that trans-GSD transitions are favoured more strongly than cis-GSD transitions, with transitions to ESD intermediate (e.g., with $|D^\male - D^\female| << |\alpha_\Delta^\male - \alpha_\Delta^\female + t^\male - t^\female|$ we have $\lambda_{W'}^{(XY)} > \lambda_{Y'}^{(XY)}$; \DIFdelbegin \DIFdel{Eqs \ref{eq:lambda_neoW}and \ref{eq:freq_diffs}}\DIFdelend \DIFaddbegin \DIFadd{Eq \ref{eq:lambda_neoW}}\DIFaddend ). 
...
%DIFDELCMD < %%%
\DIFdel{In support of their role in sex chromosome turnover, genes expected to be under sexually-antagonistic selection }\DIFdelend \DIFaddbegin \DIFadd{To further examine this link, future theoretical investigations could also include selfing and inbreeding depression during transitions between dioecy and hermaphroditism. 
Future empirical investigations could look for evidence of haploid selection acting on former sex chromosomes in hermaphroditic species }\DIFaddend (e.g., \DIFdelbegin \DIFdel{those causing bright male colouration)have been found }\DIFdelend \DIFaddbegin \DIFadd{a study such as \mbox{%DIFAUXCMD
\cite{Sandler264382} }%DIFAUXCMD
on ancestral, rather than derived, sex chromosomes). 
}

\DIFadd{New sex-determining alleles have previously be shown to spread when they arise in linkage with loci that experience sex differences in selection because beneficial associations build up between alleles that determine sex and alleles that are favoured in that sex~\mbox{%DIFAUXCMD
...
\DIFdelbegin \DIFdel{Our results show, however, thattight ancestral-linkage of polymorphic loci , maintained by sexually-antagonistic selection or otherwise, can also }\DIFdelend \DIFaddbegin \DIFadd{However, we show that, if selected loci are linked to the ancestral sex-determining locus, they can }\DIFaddend drive trans-GSD transitions \DIFaddbegin \DIFadd{during which sex-linkage is reduced (Conclusion 1), thus widening the genomic locations that could be driving trans-GSD transitions}\DIFaddend .
...
\DIFadd{Our tight linkage results are consistent with empirical data from species in which new feminizing mutations are found alongside ancestral XY loci. 
...
\DIFdelbegin \DIFdel{For example, in }\DIFdelend \DIFaddbegin \DIFadd{In }\DIFaddend nine \textit{Akadon} \DIFaddbegin \DIFadd{rodent }\DIFaddend species, it appears that male-determining-\textit{sry} expression is suppressed by an autosomal feminizing allele, creating XY females~\cite{Bianchi2002,Sanchez2010}\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{. 
XY females }\DIFaddend have increased fitness relative to XX females~\cite{Hoekstra2001}. \DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \DIFadd{However, it is not yet clear whether loci linked to the feminizing factor or the ancestral Y cause this effect. 
Most convincingly, in }\DIFaddend \textit{Mus microtoides}, females can have XX, XX$^\ast$ or X$^\ast$Y genotypes~\cite{Veyrunes2010}. 
...
Although Y-linkage of female-beneficial alleles is counterintuitive, our model suggests that it can be stably maintained and then favour new feminizing mutations, which \DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{would be }\DIFaddend a parsimonious explanation for the spread of feminizing alleles in \DIFdelbegin \DIFdel{these rodent species}\DIFdelend \DIFaddbegin \DIFadd{this case}\DIFaddend . 
...
show that }\DIFdelend \DIFaddbegin \DIFadd{It should be noted, however, }\DIFaddend that \DIFdelbegin \DIFdel{neo-Y/neo-W alleles can be favoured when they arise near to haploid and/or diploid selected loci, which also occurs in our model}\DIFdelend \DIFaddbegin \DIFadd{the dynamics of sex-determining alleles with very weak effect will be influenced by genetic drift, which itself has been shown to favour trans- over cis-GSD transitions when there is no selection \mbox{%DIFAUXCMD
...
In particular, both can select for new sex-determining loci that are more loosely linked to loci under selection \DIFaddbegin \DIFadd{(Conclusions 1 \& 2B)}\DIFaddend . 
In addition, haploid selection can cause transitions analogous to those caused by purely sexually-antagonistic selection, eliminating the need for differences in selection between male and female diploids \DIFaddbegin \DIFadd{(Conclusion 2A)}\DIFaddend .
...
\DIFdelbegin \DIFdel{Perhaps counterintuitively}\DIFdelend \DIFaddbegin \DIFadd{Further}\DIFaddend , transitions involving haploid selection can be driven by \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend selection or cause \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend biases to evolve; \DIFdelbegin \DIFdel{we do not find Fisherian sex ratio selection to be }\DIFdelend \DIFaddbegin \DIFadd{Fisherian sex-ratio selection is not }\DIFaddend an overwhelming force \DIFaddbegin \DIFadd{(Conclusions 3 \& 4)}\DIFaddend . 
...
We thank Georgy Sandler and Stephen Wright for sharing their results with us and we thank Bret Payseur for helpful comments on this manuscript. 
