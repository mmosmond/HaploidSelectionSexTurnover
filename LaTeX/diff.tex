...
%DIF ADD main_revised.tex   Wed Mar 28 15:33:27 2018
...
(2) We \DIFdelbegin \DIFdel{also }\DIFdelend consider selection upon haploid genotypes either during gametic competition (e.g., pollen/sperm competition \DIFaddbegin \DIFadd{involving multiple males}\DIFaddend ) or meiosis (i.e., non-Mendelian segregation), which can cause the zygotic sex ratio to become biased. 
...
Furthermore, \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend biases may increase or decrease with the spread of new sex chromosomes. 
...
These results predict conditions under which sex-determining systems are likely to be labile and draw \DIFdelbegin \DIFdel{novel connections with sex ratio }\DIFdelend \DIFaddbegin \DIFadd{connections with sex-ratio }\DIFaddend evolution.
...
Our models extend these theories to include selection upon haploid genotypes (meiotic drive or gametic competition), which causes \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend biases and occurs differently in males and females. 
...
\DIFaddbegin \DIFadd{In sum, accumulating evidence indicates that transitions between sex-determining systems are common~\mbox{%DIFAUXCMD
...
Predominant theories accounting for \DIFdelbegin \DIFdel{the spread of new sex-determining }\DIFdelend \DIFaddbegin \DIFadd{transitions between GSD }\DIFaddend systems by selection involve \DIFaddbegin \DIFadd{sex-ratio selection or }\DIFaddend fitness differences between sexes (e.g., sexually antagonistic selection)\DIFdelbegin \DIFdel{or sex ratio selection}\DIFdelend ~\cite{Blaser2012, Beukeboom:2014vb,vanDoorn2014re}. 
...
\DIFdelbegin \DIFdel{van Doorn and Kirkpatrick~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu} }%DIFAUXCMD
and Muralidhar and Veller~\mbox{%DIFAUXCMD
\cite{Muralidhar2018} }%DIFAUXCMD
have shown that new }\DIFdelend \DIFaddbegin \DIFadd{`Fisherian' sex-ratio selection favours a 1:1 zygotic sex ratio when assuming that males and females are equally costly to produce~\mbox{%DIFAUXCMD
\cite{Fisher:1930wy,Charnov:1982wg}}%DIFAUXCMD
.
This follows from the fact that, for an autosomal locus, half of the genetic material is inherited from a male and half from a female~\mbox{%DIFAUXCMD
\cite{West:2009we}}%DIFAUXCMD
. 
...
Because the sex ratio is directly determined by the }\DIFaddend sex-determining \DIFdelbegin \DIFdel{alleles can be favoured if they arise at loci }\DIFdelend \DIFaddbegin \DIFadd{system, it has been suggested that sex-ratio selection is a particularly dominant force in the evolution of sex determination (e.g., Bull, 1983, p 66-67 ~\mbox{%DIFAUXCMD
\cite{Bull:1983vi}}%DIFAUXCMD
; Buekeboom and Perrin, 2014, Chapter 7~\mbox{%DIFAUXCMD
\cite{Beukeboom:2014vb}}%DIFAUXCMD
). 
}

%DIF > MAJOR MECHANISMS
%DIF > (2) sex-antagonism
\DIFadd{New genetic sex-determining alleles have been shown to be favoured in they arise }\DIFaddend in close linkage with a locus that experiences sexual antagonism \DIFdelbegin \DIFdel{. 
...
}\DIFaddend Tighter linkage allows a stronger favourable association to build up between a male-beneficial allele, and a neo-Y allele, for example. 
...
However, any sexually-antagonistic loci that are more closely linked to the ancestral sex-determining locus will develop similar, favourable associations and are expected to hinder the spread of a new sex-determining system\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu}}%DIFAUXCMD
}\DIFaddend . 

\DIFdelbegin \DIFdel{The sex ratio is directly determined by the }\DIFdelend %DIF > Tight linkage Model Feature and Result
\DIFaddbegin \DIFadd{One novel feature of the models developed here is that we explicitly \textcolor{red}{(is this wording fair to vD\&K?)} consider loci in very tight linkage with the ancestral }\DIFaddend sex-determining \DIFdelbegin \DIFdel{system, and it has therefore been suggested that sex ratio selection is a dominant force in the evolution of sex determination }\DIFdelend \DIFaddbegin \DIFadd{locus }\DIFaddend (e.g., \DIFdelbegin \DIFdel{Bull, 1983, p 66-67 ~\mbox{%DIFAUXCMD
\cite{Bull:1983vi}}%DIFAUXCMD
; Buekeboom andPerrin, 2014, Chapter 7~\mbox{%DIFAUXCMD
\cite{Beukeboom:2014vb}}%DIFAUXCMD
). 
`Fisherian' sex ratio selection favours a 1:1 zygotic sex ratio when assuming that males and females are equally costly to produce~\mbox{%DIFAUXCMD
\cite{Fisher:1930wy,Charnov:1982wg}}%DIFAUXCMD
. 
This follows from the fact that, for an autosomal locus , half of the genetic material is inherited from a male and half from a female~\mbox{%DIFAUXCMD
\cite{West:2009we}}%DIFAUXCMD
...
Therefore, a mutant that increases investment in }\DIFdelend \DIFaddbegin \DIFadd{within the non-recombining region of a sex chromosome). %DIF >  in the results section: `Tight linkage with the ancestral sex-determining locus'). 
Our analysis shows that, under various forms of selection, }\DIFaddend the \DIFdelbegin \DIFdel{rarer sex will spread via the higher per-individual contributions made by that sex. 
...
In the case of sex-chromosome evolution, Kozielska }\DIFdelend \DIFaddbegin \DIFadd{ancestral-X can carry suboptimal alleles for females and/or the ancestral-Y can carry female-beneficial alleles. 
...
\DIFadd{Most significantly, we include haploid selection (gametic competition or meiotic drive) in models describing cis-GSD, trans-GSD, and GSD to ESD transitions, which poses an apparent evolutionary problem. 
...
. 
Therefore, one might expect new sex-determining systems to benefit from close linkage with haploid selected loci, as found for sex-differences in diploid selection~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu, Muralidhar2018}}%DIFAUXCMD
. 
...
Ubeda }\DIFaddend et al. (\DIFdelbegin \DIFdel{2014)~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm} }%DIFAUXCMD
consider systems in which the ancestral sex chromosomes experience meiotic drive (e. 
g., where driving X or Y chromosomes are inherited disproportionately often), which causes }\DIFdelend \DIFaddbegin \DIFadd{2015)~\mbox{%DIFAUXCMD
\cite{Ubeda:2015fx}}%DIFAUXCMD
, considers ancestral-ESD (with no sex-ratio bias) and numerically show that new GSD alleles can spread if they arise in linkage with meiotic drive loci. 
For example, a masculinizing allele spreads in association with an allele that is favoured during male meiosis, causing }\DIFaddend sex ratios to become \DIFdelbegin \DIFdel{biased~\mbox{%DIFAUXCMD
\cite{Hamilton:1967ts}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{male-biased and thereby suggesting that such beneficial associations overwhelm selection to balance the sex ratio. 
...
They find that \DIFdelbegin \DIFdel{masculinizing or feminizing mutations that arise at a new, unlinked locus (i.e., neo-Y or neo-W chromosomes) can then spread }\DIFdelend \DIFaddbegin \DIFadd{new}\DIFaddend , \DIFdelbegin \DIFdel{which restore an even }\DIFdelend \DIFaddbegin \DIFadd{completely unlinked, GSD systems can spread if they generate the rarer sex, creating a balanced }\DIFaddend sex ratio. 
\DIFaddbegin \DIFadd{That is, they conclude that Fisherian sex-ratio selection caused by haploid selection favours new GSD systems. 
}\DIFaddend 

\DIFdelbegin \DIFdel{Here we use mathematical models to }\DIFdelend %DIF > main results
...
Even in animal and plant species that have much larger and more conspicuous diploid phases than haploid phases, many loci experience significant haploid selectionthrough gamete competition and/or meiotic drive~\mbox{%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{Segregation distortion provides putative evidence of haploid selection and can sometimes be attributed to meiotic drive and/or gametic competition~\mbox{%DIFAUXCMD
...
If we assume that }\DIFdelend \DIFaddbegin \DIFadd{GSD or ESD systems spread with generic linkage between the loci involved (and generic sex-specific haploid and diploid selection). 
...
\cite{vanDoorn:2007eu,vanDoorn:2010hu} }%DIFAUXCMD
and suggests that }\DIFaddend haploid selection \DIFdelbegin \DIFdel{at any particular locus predominantly occurs in one sex (e. 
g., meiotic drive during spermatogenesis), then such loci experience a form of sex-specific selection. 
In this respect, we might expect }\DIFdelend \DIFaddbegin \DIFadd{is more likely to promote transitions between sex-determination systems. 
...
In particular, under a wide range of conditions, we show }\DIFaddend that haploid selection \DIFdelbegin \DIFdel{would affect transitions between }\DIFdelend \DIFaddbegin \DIFadd{drives transitions in }\DIFaddend sex-determining \DIFdelbegin \DIFdel{systems in a similar manner to sex-specific diploid selection (as explored in~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu}}%DIFAUXCMD
). 
...
For example, there are several known cases of sex ratio bias caused by sex-linked meiotic drive alleles (Burt and Trivers, 2006, Chapter 6~\mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{system }\textit{\DIFadd{equally strongly}} \DIFadd{whether sex-ratio biases increase or decrease (and whether sex-ratio biases are present or absent). 
...
}\DIFdelend %DIF > Here we use mathematical models to find the conditions under which new sex-determining systems spread when individuals experience selection at both diploid and haploid stages, which allows fitness differences between the sexes and sex ratio biases to occur simultaneously
\DIFaddbegin 
...
\DIFadd{\textcolor{red}{move to discussion somewhere:
...
\DIFadd{\textcolor{red}{move to results somewhere:
...
\DIFaddend %Because sex-determining loci are often found within a region of suppressed recombination, there can be a significant number of tightly linked loci. 
...
}\DIFdelend %DIF > We find that loci tightly linked with the sex-determining locus can drive transitions in which the heterogametic sex changes, even when the new sex-determining locus is less closely linked to loci under selection (either including haploid selection or not). 
...
%life-cycle
\DIFaddbegin \DIFadd{A full description of our model, including recursion equations, is given in }\nameref{app:recurs}\DIFadd{. 
}\DIFaddend In each generation, we census the genotype frequencies in male and female gametes/gametophytes (hereafter gametes) before gametic competition. 
\DIFdelbegin \DIFdel{A full description of our model, including recursion equations, is given in }%DIFDELCMD < \nameref{app:recurs}%%%
\DIFdel{. 
}\DIFdelend First, competition occurs among male gametes (sperm/pollen competition) and among female gametes (egg/ovule competition) separately. 
Selection during gametic competition depends on the $\mathbf{A}$ locus genotype; relative fitnesses are given by \DIFdelbegin \DIFdel{$w_A^\Hermaphrodite$ and $w_a^\Hermaphrodite$ ($\Hermaphrodite \in \{\female,\male\}$}\DIFdelend \DIFaddbegin \DIFadd{$w_A^\circ$ and $w_a^\circ$ ($\circ \in \{\female,\male\}$}\DIFaddend ; see table \ref{tab:fitnesstable}). %$w_{A}^\male$ and $w_{a}^\male$ for male gametes and $w_{A}^\female$ and $w_{a}^\female$ for female gametes, see table \ref{tab:fitnesstable}. 
...
\DIFaddbegin \DIFadd{The model is entirely deterministic and hence ignores chance fluctuations in allele frequencies due to genetic drift.
}\DIFaddend 
...
\DIFaddend The evolution of a new sex-determining system requires that a rare mutant allele, $m$, at the novel sex-determining locus, $\mathbf{M}$, increases in frequency when rare. 
...
selection on loci within the non-recombining region around the sex-determining locus can favour the invasion of a less closely linked neo-W, whatever the }\DIFdelend \DIFaddbegin \DIFadd{In particular,
}\vspace{0.5cm}

\noindent\textit{\DIFadd{Conclusion 1A:}}
\textbf{\DIFadd{selection on loci within the non-recombining region around the sex-determining locus ($r\approx0$) allows the invasion of less closely linked sex-determining alleles ($r<R\leq1/2$) during a trans-GSD transition}}\DIFadd{.
%DIF > \textcolor{red}{[new result for sex-antagonism and overdominance, but already shown by Kozielska for ploidy-antagonism]}
}\vspace{0.5cm}
...
The invasion of completely unlinked neo-W alleles ($R = 1/2$) can occur with overdominance in males or with haploid selection \DIFaddbegin \DIFadd{(as shown for the case of male drive opposed by diploid selection in \mbox{%DIFAUXCMD
\cite{Kozielska:2010vm}}%DIFAUXCMD
) }\DIFaddend but is not possible with only sexually-antagonistic selection if selection is directional in each diploid sex (see \nameref{file:Mathematica}).
%DIF > \vspace{0.5cm}
\DIFaddbegin 
...
\caption{
{\bf \DIFaddFL{Transitions between XY and ZW systems can occur even when the new sex-determining locus is less tightly linked to a locus under sexually-antagonistic selection (even without haploid selection).}}
\DIFaddFL{In panel A, linkage is loose enough relative to selection that the analytical results assuming weak selection hold, and a neo-W allele can only invade when it arises at a locus more tightly linked with the selected locus ($R<r$; shaded region).
...
\DIFaddend To develop an intuition for how less closely linked neo-W alleles invade ($R>r$), we here focus on cases where there is no haploid selection and discuss the additional effect of haploid selection in \nameref{app:inv_cond}. 
...
%DIFDELCMD < %%%
\DIFdelend If we categorise the $a$ allele as being ancestrally `male-beneficial' via the fact that it is fixed on Y backgrounds, then $\Lambda_{W'A}^{(XY)}>1$ indicates that the neo-W spreads when found with the ancestrally `female-beneficial' allele. 
...
In panel A, male drive ($\alpha^\male_{\Delta} = -1/20$, \DIFdelbeginFL \DIFdelFL{$t^\Hermaphrodite = \alpha^\female_{\Delta} = 0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t^\circ = \alpha^\female_{\Delta} = 0$}\DIFaddendFL ) opposes selection in diploids (no sex-differences: \DIFdelbeginFL \DIFdelFL{$s^\Hermaphrodite = 1/10$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$s^\circ = 1/10$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$h^\Hermaphrodite = 7/10$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$h^\circ = 7/10$}\DIFaddendFL ), in which case the new sex-determining allele can invade regardless of its linkage with the selected locus ($R$).   
In panel B, gametic competition in males ($t^\male = -1/10$, \DIFdelbeginFL \DIFdelFL{$t^\female = \alpha^\Hermaphrodite_{\Delta} = 0$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t^\female = \alpha^\circ_{\Delta} = 0$}\DIFaddendFL ) opposes selection in diploids (sex-differences: $s^\male = 3/20$, $s^\female = 1/20$, \DIFdelbeginFL \DIFdelFL{$h^\Hermaphrodite = 7/10$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$h^\circ = 7/10$}\DIFaddendFL ), in which case the new sex-determining allele can once again invade regardless of $R$.
}
...
This \DIFdelbegin \DIFdel{can allow neo-W invasion ($\lambda_{W'}^{(XY)}>1$), 
even when the new sex-determining locus is less closely linked to the selected locus ($R>r$). 
}\DIFdelend \DIFaddbegin \DIFadd{implies, 
}\vspace{0.5cm}

\noindent\textit{\DIFadd{Conclusion 1B:}}\DIFadd{\
}\textbf{\DIFadd{when there is an intermediate rate of recombination between the ancestral sex-determining allele and a selected locus ($0<r<1/2$), haploid selection allows less closely linked sex-determining alleles ($r<R$) to invade during trans-GSD transitions ($\lambda_{W'}^{(XY)}>1$).}}
%DIF > \textcolor{red}{[new result because Kozielska had ancestrally-tight sex-linkage and Ubeda had ESD]}
\vspace{0.5cm}
...
Previous research \DIFdelbegin \DIFdel{suggests }\DIFdelend \DIFaddbegin \DIFadd{has shown }\DIFaddend that when the ancestral sex-determining locus is linked to a locus that experiences haploid selection (e.g., meiotic drive), alleles at a new, unlinked sex-determining locus \DIFdelbegin \DIFdel{invade to }\DIFdelend \DIFaddbegin \DIFadd{can invade and }\DIFaddend restore equal sex ratios~\cite{Kozielska:2010vm}. 
...
}\DIFdelend \DIFaddbegin \DIFadd{, implying that
}\vspace{0.5cm}

\noindent\textit{\DIFadd{Conclusion 2:}}
\textbf{\DIFadd{when selection is weak relative to recombination, sex-ratio selection is exactly equal in magnitude to the selection imposed by associations with selected alleles.}}
%DIF > \textcolor{red}{[new result]}
\vspace{0.5cm}

\noindent \DIFaddend For example, in Fig \ref{fig:SexRatioBad}A neo-W alleles invade an ancestral-XY system where females are initially rare, equalizing the sex ratio.
However, Fig \ref{fig:SexRatioBad}B shows that a neo-Y can invade the resulting ZW system under the same conditions. 
When $R<1/2$, the invading neo-Y becomes associated with the male meiotic drive allele and the zygotic sex ratio actually evolves to become male-biased \DIFaddbegin \DIFadd{(as in the scenarios considered in \mbox{%DIFAUXCMD
\cite{Kozielska:2010vm,Ubeda:2015fx}}%DIFAUXCMD
)}\DIFaddend .
...
%DIF > What we would like to stress is that sex-ratio selection alone cannot predict when new sex-determining systems can evolve.
Significantly, with weak selection, we find that there is no difference in conditions allowing XY to ZW and ZW to XY transitions, indicating that sex chromosome transitions are not predominantly predicted by their effect on the sex ratio (i.e., the \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend bias created by male haploid selection facilitates the spread of a neo-W allele into an XY system to the same degree that male haploid selection drives the spread of a neo-Y into a ZW system with a 1:1 sex ratio). 
...
\DIFaddbegin \DIFadd{To further examine this link, future investigations could examine the combined effects of haploid pollen competition, selfing, and inbreeding depression in allowing transitions between dioecy and hermaphroditism. 
}\DIFaddend 
...
show that }\DIFdelend \DIFaddbegin \DIFadd{It should be noted, however, }\DIFaddend that \DIFdelbegin \DIFdel{neo-Y/neo-W alleles can be favoured when they arise near to haploid and/or diploid selected loci, which also occurs in our model}\DIFdelend \DIFaddbegin \DIFadd{the dynamics of sex-determining alleles with very weak effect will be influenced by genetic drift, which itself has been shown to cause trans-GSD transitions \mbox{%DIFAUXCMD
