...
%DIF DEL main.tex           Sun Mar 25 06:45:15 2018
%DIF ADD main_revised.tex   Mon May 21 10:22:42 2018
...
Furthermore, sex ratio }\DIFdelend \DIFaddbegin \DIFadd{either strengthened or weakened. 
...
Furthermore, we find that sex-ratio }\DIFaddend biases may increase or decrease with the spread of new sex chromosomes\DIFdelbegin \DIFdel{. 
Thus, we find }\DIFdelend \DIFaddbegin \DIFadd{, which implies }\DIFaddend that transitions between sex-determining systems cannot be simply predicted by selection to equalise the sex ratio.
\DIFdelbegin \DIFdel{Overall, our models reveal that transitions between sex-determining systems, particularly transitions where the heterogametic sex changes, can be driven by loci in previously unpredicted genomic locations that experience selection during diploid and/or haploid phases.
...
}\DIFdelend \DIFaddbegin \DIFadd{In fact, under many conditions, we find that transitions in sex determination are favoured equally strongly in cases where the sex ratio bias increases or decreases.
...
Therefore, our models predict }\DIFdelend \DIFaddbegin \DIFadd{can spread despite being less closely linked to selected loci as long as initial linkage is tight or haploid selection is present.
...
Within several taxa, the chromosome that harbours the master sex-determining locus changes\DIFdelbegin \DIFdel{. 
For example, transitions }\DIFdelend \DIFaddbegin \DIFadd{, due either to translocation }\DIFaddend of the master sex-determining locus \DIFdelbegin \DIFdel{between chromosomes or }\DIFdelend \DIFaddbegin \DIFadd{or to }\DIFaddend the evolution of \DIFdelbegin \DIFdel{new master sex-determining loci where }\DIFdelend \DIFaddbegin \DIFadd{a new master locus.  
During these transitions, }\DIFaddend the heterogametic sex \DIFdelbegin \DIFdel{does not change }\DIFdelend \DIFaddbegin \DIFadd{can remain the same }\DIFaddend (hereafter `cis-GSD transitions') \DIFdelbegin \DIFdel{have occurred }\DIFdelend \DIFaddbegin \DIFadd{as }\DIFaddend in Salmonids~\cite{Li:2011fm,Yano:2012di}, Diptera~\cite{Vicoso:2015hf}, and \textit{Oryzias}~\cite{Myosho:2012fv}. 
...
}\DIFdelend \DIFaddbegin \DIFadd{, including new sex determination systems.
...
\DIFdelbegin \DIFdel{Here we use mathematical models to find the conditions under which new }\DIFdelend %DIF > MAJOR MECHANISMS
%DIF > (2) sex-antagonism
\DIFaddbegin \DIFadd{The evolution of sex determination is also thought to be strongly influenced by differences in selection between the sexes ~\mbox{%DIFAUXCMD
\cite{Blaser2012, Beukeboom:2014vb,vanDoorn2014re}}%DIFAUXCMD
.
For example, loci experiencing sexual antagonism have been shown to favour the spread of new genetic }\DIFaddend sex-determining \DIFdelbegin \DIFdel{systems spread when individuals experience selection at both diploid and haploid stages, which allows fitness differences between the sexes and sex ratio biases to occur simultaneously. 
...
Gametic competition may be particularly common in plants, in which 60-70\% of all genes are expressed in the male gametophyte, and these genes exhibit stronger signatures of selection than randomly-chosen genes~\mbox{%DIFAUXCMD
\cite{Borg:2009jpa,Arunkumar:2013iq,Gossmann:2014dua}}%DIFAUXCMD
}\DIFdelend %DIF > Tight linkage Model Feature and Result
\DIFaddbegin \DIFadd{One novel feature of the models developed here is that we explicitly consider the maintenance of genetic variation around the ancestral sex-determining locus (e.g., within the non-recombining region of a sex chromosome). %DIF > during trans-GSD transitions
...
In addition, \DIFdelbegin \DIFdel{artificial selection pressures applied to male gametophytes are known to cause a response to selection(e.g., ~\mbox{%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{selection on ancestral-X chromosomes in males can prevent the X from becoming optimally specialised for female-beneficial alleles. 
These factors, in turn, can favour a new ZW sex-determining locus that has weaker linkage with loci under selection, which was not revealed by previous theory \mbox{%DIFAUXCMD
\cite{vanDoorn:2010hu}}%DIFAUXCMD
. A similar argument applies to ZW$\rightarrow$XY transitions. 
Thus, we show that selected loci in very tight linkage with the ancestral GSD locus can favour trans-GSD transitions during which linkage associations are actually weakened}\DIFaddend . 

\DIFdelbegin \DIFdel{There are various ways by which genes experiencing haploid selection could influence transitionsbetween sex-determining systems.
If we assume that haploid selection at any particular locus predominantly occurs in one sex (e. 
g., meiotic drive during spermatogenesis), then such loci experience a form of sex-specific selection}\DIFdelend %DIF > Haploid selection model feature and discussion of why needed
\DIFaddbegin \DIFadd{Most significantly, we include haploid selection (gametic competition or meiotic drive) in models describing cis-GSD, trans-GSD, and GSD to ESD transitions}\DIFaddend .
\DIFdelbegin \DIFdel{In this respect, we might expect that haploid selection would affect transitions }\DIFdelend \DIFaddbegin \DIFadd{This poses an apparent evolutionary problem. 
On one hand, haploid selection is typically sex-limited in that it usually occurs among gametes produced by one sex only~\mbox{%DIFAUXCMD
\cite{Mulcahy:1996ha,JOSEPH:2004haa,Ubeda:2005gw,Lindholm:2016cw}}%DIFAUXCMD
. 
Therefore, one might expect new sex-determining systems to benefit from close linkage with haploid selected loci, as found for loci that experience diploid-sex-differences in selection~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu, Muralidhar2018}}%DIFAUXCMD
. 
On the other hand, associations }\DIFaddend between sex-determining \DIFdelbegin \DIFdel{systems in a similar manner to sex-specific diploid selection (as explored in~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu}}%DIFAUXCMD
) }\DIFdelend \DIFaddbegin \DIFadd{loci and haploid selected loci generate biased zygotic sex ratios, which should generally hinder the spread of new sex-determining systems}\DIFaddend . 
\DIFdelbegin \DIFdel{That is, new masculinizing mutations (neo-Y alleles) could be favoured via associations with alleles that are beneficial in the male haploid stage. On the other hand, sex ratios can also become biased by linkage between the sex-determining locus and a locus that harbours genetic variation in haploid fitness. 
}\DIFdelend \DIFaddbegin 

%DIF > Previous models with haploid selection
\DIFadd{Two previous studies have considered the spread of GSD with sex-limited meiotic drive~\mbox{%DIFAUXCMD
...
considered ancestral-ESD (with no sex-ratio bias) and numerically showed that new GSD alleles can spread if they arise in linkage with meiotic drive loci. 
}\DIFaddend For example, \DIFdelbegin \DIFdel{there are several known cases of sex ratio bias caused by sex-linked meiotic drive alleles (Burt and Trivers, 2006, Chapter 6~\mbox{%DIFAUXCMD
\cite{Burt:2006}}%DIFAUXCMD
) or selection among X- and Y-bearing pollen~\mbox{%DIFAUXCMD
\cite{Lloyd:1974tz,Conn:1981uw,Stehlik:2005ul,Stehlik:2006to,Field:2012fd,Field:2013cc}}%DIFAUXCMD
. 
It is not immediately clear how the spread of }\DIFdelend \DIFaddbegin \DIFadd{a masculinizing allele spreads in association with an allele that is favoured during male meiosis, causing sex ratios to become male-biased.
This suggests that the benefits of associating with driving alleles can overwhelm selection to balance the sex ratio. %DIF > , at least when new sex-determining alleles are rare. 
However, Kozielska et al. (2010)~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm} }%DIFAUXCMD
considered an ancestral GSD system that is perfectly linked to a meiotic driver and therefore exhibiting an ancestral sex ratio bias. 
...
It is thus currently unclear when haploid selection favors increased versus decreased linkage between haploid selected loci and a }\DIFaddend new \DIFaddbegin \DIFadd{sex-determining locus. 
In addition, because the sex ratio is determined by linkage between haploid selected loci and the }\DIFaddend sex-determining \DIFaddbegin \DIFadd{locus, it is also unclear when Fisherian sex-ratio selection is the most important driver of transitions between sex-determining }\DIFaddend systems\DIFdelbegin \DIFdel{would be influenced by the combination of sex ratio biases and associations with haploid selected alleles. 
}\DIFdelend \DIFaddbegin \DIFadd{. 
}\DIFaddend 

\DIFdelbegin \DIFdel{We find that the spread of novel sex-determining systems }\DIFdelend %DIF > main results
\DIFaddbegin \DIFadd{Here, we analytically find the conditions under which new GSD or ESD systems spread in ancestral GSD systems with any degree of linkage between the loci involved and arbitrary forms of haploid and diploid selection. 
Doing so, we reconcile and generalize the results of Kozielska et al. (2010)~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm} }%DIFAUXCMD
and Ubeda et al. (2015)~\mbox{%DIFAUXCMD
\cite{Ubeda:2015fx} }%DIFAUXCMD
by deriving conditions for the spread of new GSD systems that alter linkage with haploid selected loci.
Our result is qualitatively distinct from those for diploid selection alone~\mbox{%DIFAUXCMD
\cite{vanDoorn:2007eu,vanDoorn:2010hu} }%DIFAUXCMD
and suggests that haploid selection }\DIFaddend is \DIFdelbegin \DIFdel{influenced by both Fisherian sex ratio selection and by selection on linked alleles. 
...
In addition to considering haploid selection , another novel development in our model is that we consider loci that are in very tight linkage with the ancestral }\DIFdelend \DIFaddbegin \DIFadd{more likely to promote transitions between sex-determination systems. 
We also show that transitions involving haploid selection cannot be simply explained by invoking sex-ratio selection. 
In particular, under a wide range of conditions, we show that transitions in }\DIFaddend sex-determining \DIFdelbegin \DIFdel{locus. 
...
We find that loci tightly linked with the }\DIFdelend \DIFaddbegin \DIFadd{system are favoured }\textit{\DIFadd{equally strongly}} \DIFadd{in situations where sex-ratio biases increase or decrease (and in situations where sex-ratio biases are ancestrally present or absent). 
...
\DIFaddbeginFL \centerline{\includegraphics[width=0.9\linewidth]{../Plots/Sex_determination_outline.pdf}}
\DIFaddendFL \caption{
...
\DIFdelbeginFL \DIFdelFL{If $r<1/2$, then associations between ancestral sex-determining alleles and selected alleles can be maintained past recombination in males. 
}\DIFdelendFL %DIF > If $r<1/2$, then associations between ancestral sex-determining alleles and selected alleles can be present in new male gametes/gametophytes. 
Panel B: Haploid selection is often \DIFdelbeginFL \DIFdelFL{sex-specific}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sex-limited}\DIFaddendFL , occurring during haploid production or competition in \DIFdelbeginFL \DIFdelFL{either males or females. 
For example, haploid selection }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{one sex (shown here }\DIFaddendFL in males \DIFdelbeginFL \DIFdelFL{only is represented }\DIFdelendFL by \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL dashed \DIFdelbeginFL \DIFdelFL{circle}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{circles)}\DIFaddendFL . 
If X or Y alleles \DIFdelbeginFL \DIFdelFL{remain associated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{are linked }\DIFaddendFL with alleles that experience haploid selection in males ($r<1/2$), then zygotic sex ratios can become biased because either X- or Y-bearing male gametes/gametophytes will be more abundant after haploid selection. 
\DIFdelbeginFL \DIFdelFL{However, the zygotic sex ratio is not biased by male haploid selection in ZW sex-determining systems. 
}\DIFdelendFL Similarly, zygotic \DIFdelbeginFL \DIFdelFL{sex ratio }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sex-ratio }\DIFaddendFL biases can \DIFdelbeginFL \DIFdelFL{occur }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{arise }\DIFaddendFL if haploid selected alleles are \DIFdelbeginFL \DIFdelFL{associated }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{linked }\DIFaddendFL with new sex-determining alleles ($R<1/2$). 
\DIFaddbeginFL \DIFaddFL{However, the zygotic sex ratio is not biased by male haploid selection in ZW sex-determining systems. 
}\DIFaddendFL Panel C: During cis-GSD transitions (XY to XY or ZW to ZW), a neo-Y allele \DIFaddbeginFL \DIFaddFL{($m$) }\DIFaddendFL spreads to pseudo-fixation (\DIFdelbeginFL \DIFdelFL{its maximum frequency among male gametes}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{i.e, all males bear the neo-Y}\DIFaddendFL ) and the ancestral Y allele is lost. 
Panel D: During trans-GSD transitions (XY to ZW or ZW to XY), a neo-W allele \DIFaddbeginFL \DIFaddFL{($m$) }\DIFaddendFL spreads to pseudo-fixation (\DIFdelbeginFL \DIFdelFL{its maximum frequency among female gametes}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{i.e, all females bear the neo-W}\DIFaddendFL ) and the ancestral X allele is lost. 
...
Relative fitnesses in sex $\circ \in \{\female,\male\}$ }\DIFaddend during gametic competition \DIFdelbegin \DIFdel{depends on the $\mathbf{A}$ locus genotype; relative fitnesses }\DIFdelend are given by \DIFdelbegin \DIFdel{$w_A^\Hermaphrodite$ and $w_a^\Hermaphrodite$ ($\Hermaphrodite \in \{\female,\male\}$; see table }\DIFdelend \DIFaddbegin \DIFadd{$w_A^\circ$ and $w_a^\circ$ (see Table }\DIFaddend \ref{tab:fitnesstable}).
...
\DIFaddbegin \DIFadd{We do not consider fertility selection that depends on the mating partner, e.g., sexual selection with variation in choosiness. 
...
This is determined by the leading eigenvalue ($\lambda_m^{(XY)}$) of the system of eight equations describing the frequency of eggs and sperm carrying the $m$ allele in the next generation (equations \ref{eq:recursions}). %DIF < , evaluated at the above equilibrium.
%DIF < Thus, the system describing the change in frequency of the $m$ allele consists of eight recursion equations. 
This system simplifies substantially for a dominant neo-Y ($k=0$) or neo-W ($k=1$), see }%DIFDELCMD < \nameref{app:inv_cond}%%%
\DIFdel{. 
The leading eigenvalue for a rare neo-Y or neo-W allele, $m\in\{Y',W'\}$, is the largest value of X that solves $x^2+ b x + c = 0$.  %DIF < $(\lambda_m^{(i)})^2+ b \lambda_m^{(i)} + c =0$. 
...
{%DIFAUXCMD
%DIFDELCMD < {\bf %%%
\DIFdelFL{Parameters determining invasion of mutant neo-Y and neo-W alleles into an ancestrally XY system}%DIFDELCMD < }%%%
}
%DIFAUXCMD
...
%DIFDELCMD <    \noalign{\vskip 0.5ex}
%DIFDELCMD <    %%%
\DIFdelFL{$m$ is a neo-Y ($k=0$) }%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \hline \noalign{\vskip 1ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\Lambda_{Y'A}^{(XY)} = {\left( 2 \zeta \right)}^{-1} \left[\hat{p}_X^\female w_{A}^{\female} w_{A}^{\male} w_{AA}^{\male} + (1-\hat{p}_X^\female) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right) $}%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\Lambda_{Y'a}^{(XY)} = {\left( 2 \zeta \right)}^{-1} \left[(1-\hat{p}_X^\female) w_{a}^{\female} w_{a}^{\male} w_{aa}^{\male} + \hat{p}_X^\female w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male}(1 - \alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right) $ }%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\chi_{Y'A}^{(XY)} = R {\left( 2 \zeta \right)}^{-1} \left[ (1-\hat{p}_X^\female) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/  \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right)   $}%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\chi_{Y'a}^{(XY)} = R {\left( 2 \zeta \right)}^{-1} \left[   \hat{p}_X^\female w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male} (1 - \alpha_\Delta^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right)  $}%DIFDELCMD < \\ [%%%
\DIFdelFL{1ex}%DIFDELCMD < ] \hline 
%DIFDELCMD <   \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$m$ is a neo-W ($k=1$) }%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \hline \noalign{\vskip 1ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\Lambda_{W'A}^{(XY)} = {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{A}^{\female} w_{AA}^{\female}+(1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female}(1+\alpha_{\Delta}^{\female})\right]/ \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right) $ }%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\Lambda_{W'a}^{(XY)} = {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{a}^{\female} w_{aa}^{\female}+\bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female}(1-\alpha_{\Delta}^{\female})\right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right) $ }%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\chi_{W'A}^{(XY)} = R {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female} (1+\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right)  $}%DIFDELCMD < \\ [%%%
\DIFdelFL{0.5ex}%DIFDELCMD < ] \noalign{\vskip 0.5ex}
%DIFDELCMD <   %%%
\DIFdelFL{$\chi_{W'a}^{(XY)} = R {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female} (1-\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right) $ }%DIFDELCMD < \\ [%%%
\DIFdelFL{1ex}%DIFDELCMD < ]
%DIFDELCMD <   \hline \hline 
%DIFDELCMD <    \end{tabular}
%DIFDELCMD <    \begin{flushleft} 
%DIFDELCMD < %%%
\DIFdelFL{$\bar{p}^{\male}=(1-\hat{q})\hat{p}_X^\male + \hat{q}\hat{p}_Y^\male$ is the average frequency of the $A$ allele among X- and Y-bearing male gametes.
%DIF <       \item $R$ is the probability of recombination between loci $\mathbf{A}$ and $\mathbf{M}$.
}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\hat{q}_{Y}$, when determining the invasion of new sex-determining factors.
We use }\DIFaddendFL $\zeta$ \DIFdelbeginFL \DIFdelFL{is the zygotic }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{to measure the }\DIFaddendFL sex ratio (fraction male) \DIFdelbeginFL \DIFdelFL{.
$\bar{w}^{\Hermaphrodite}_{D}$ is the mean fitness of diploids of sex $\Hermaphrodite$.
$\bar{w}_H^\Hermaphrodite$ is the mean fitness of haploids from sex $\Hermaphrodite$, see }%DIFDELCMD < \nameref{tab:meanfitnesses}%%%
\DIFdelFL{. 
}%DIFDELCMD < \end{flushleft}
%DIFDELCMD <   \label{tab:haplotype_growth}
%DIFDELCMD < \end{table}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{among zygotes, which is determined by the allele frequencies and haploid selection coefficients (}\nameref{tab:meanfitnesses}\DIFadd{).
}\DIFaddend 
...
\DIFadd{We begin by describing the general conditions under which new genetic sex determining alleles can spread within a population, without explicitly specifying ancestral allele frequencies.
...
\subsection*{\DIFadd{Generic invasion by a neo-Y or neo-W}}

\DIFadd{The evolution of a }\DIFaddend new sex-determining \DIFdelbegin \DIFdel{allele }\DIFdelend \DIFaddbegin \DIFadd{system requires that a rare mutant allele, $m$, at the novel sex-determining locus, $\mathbf{M}$, }\DIFaddend increases in frequency when rare\DIFdelbegin \DIFdel{when the largest eigenvalue is greater than one ($\lambda_m^{(XY)}>1$) . 
...
\DIFadd{Invasion by a neo-Y or a neo-W primarily depends on the ``haplotypic growth rates'' (denoted by $\Lambda_{mi}^{(XY)}$) of the neo-sex determination allele $m$ on background $i\in\{A,a\}$ without accounting for loss due to recombination ($R=0$), see Table \ref{tab:haplotype_growth}.
}\DIFaddend If both haplotypic growth rates are greater than one ($\Lambda_{mA}^{(XY)}, \Lambda_{ma}^{(XY)}>1$), then the new sex-determining allele invades regardless \DIFaddbegin \DIFadd{of }\DIFaddend the rate of recombination between the new sex-determining locus and the selected locus ($R$)\DIFdelbegin \DIFdel{, see }%DIFDELCMD < \nameref{app:inv_cond} %%%
\DIFdel{for details}\DIFdelend .
...
Furthermore, it can be shown that the leading eigenvalue declines with \DIFaddbegin \DIFadd{recombination rate, }\DIFaddend $R$, and invasion requires that $R$ is sufficiently small such that: 
...
\end{equation}
\noindent 
\DIFdelbegin \DIFdel{This condition }\DIFdelend \DIFaddbegin \DIFadd{Here $\chi_{mi}^{(XY)}>0$ is the rate at which mutant haplotypes on background $i\in\{A,a\}$ recombine onto the other $\mathbf{A}$ locus background in heterozygotes (which is proportional to $R$,  see Table \ref{tab:haplotype_growth}).
This is a ``dissociative force" that breaks down linkage disequilibrium.
}

\DIFadd{Condition \ref{eq:lambdasGen} }\DIFaddend may or may not be satisfied for the full range of locations of the new sex-determining locus, including $R=1/2$ \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{(e.g., on an autosome), }\DIFaddend depending on the nature of selection.  
Interpreting this condition, if we assume that only the $mA$ haplotype would increase in frequency when $R=0$ (\DIFaddbegin \DIFadd{i.e., }\DIFaddend $\Lambda_{ma}^{(XY)}<1<\Lambda_{mA}^{(XY)}$)\DIFaddbegin \DIFadd{, then }\DIFaddend the first term on the left-hand side of \eqref{eq:lambdasGen} is negative and invasion requires that \DIFdelbegin \DIFdel{rate of growth of }\DIFdelend \DIFaddbegin \DIFadd{growth rate of }\DIFaddend $mA$ haplotypes (\DIFdelbegin \DIFdel{$\Lambda_{mA}^{(i)}-1>0$}\DIFdelend \DIFaddbegin \DIFadd{$\Lambda_{mA}^{(XY)}-1>0$}\DIFaddend ) and the rate at which \DIFdelbegin \DIFdel{$mA$ haplotypes }\DIFdelend \DIFaddbegin \DIFadd{they }\DIFaddend are produced by recombination \DIFdelbegin \DIFdel{in $ma$ haplotypes ($\chi_{ma}^{(i)}$}\DIFdelend \DIFaddbegin \DIFadd{($\chi_{ma}^{(XY)}$}\DIFaddend ) are sufficiently large relative to the rate of decline of $ma$ haplotypes (\DIFdelbegin \DIFdel{$1-\Lambda_{ma}^{(i)}>0$}\DIFdelend \DIFaddbegin \DIFadd{$1-\Lambda_{ma}^{(XY)}>0$}\DIFaddend ) and the rate \DIFdelbegin \DIFdel{of loss of $mA$ haplotypes due to recombination }\DIFdelend \DIFaddbegin \DIFadd{at which $m$ and $A$ are dissociated by recombination ($\chi_{mA}^{(XY)}$).
}
...
\smallskip
%DIF > \caption{Parameters determining invasion (equation \ref{eq:charpoly_neoY}) for neo-Y or neo-W alleles}
\caption{ {\bf \DIFaddFL{Parameters determining invasion of mutant neo-Y and neo-W alleles into an ancestrally XY system}}}
\begin{tabular}{l}
\hline\hline
   \noalign{\vskip 0.5ex}
   \DIFaddFL{$m$ is a neo-Y }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{$\chi_{mA}^{(i)}$) }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$k=0$) }\\ [\DIFaddFL{0.5ex}] \hline \noalign{\vskip 1ex}
  \DIFaddFL{$\Lambda_{Y'A}^{(XY)} = {\left( 2 \zeta \right)}^{-1} \left[\hat{p}_X^\female w_{A}^{\female} w_{A}^{\male} w_{AA}^{\male} + (1-\hat{p}_X^\female) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right) $}\\ [\DIFaddFL{0.5ex}] \noalign{\vskip 0.5ex}
  \DIFaddFL{$\Lambda_{Y'a}^{(XY)} = {\left( 2 \zeta \right)}^{-1} \left[(1-\hat{p}_X^\female) w_{a}^{\female} w_{a}^{\male} w_{aa}^{\male} + \hat{p}_X^\female w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male}(1 - \alpha_{\Delta}^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right) $ }\\ [\DIFaddFL{0.5ex}] \noalign{\vskip 0.5ex}
  \DIFaddFL{$\chi_{Y'A}^{(XY)} = R {\left( 2 \zeta \right)}^{-1} \left[ (1-\hat{p}_X^\female) w_{a}^{\female} w_{A}^{\male} w_{Aa}^{\male} (1+\alpha_{\Delta}^{\male}) \right]/  \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right)   $}\\ [\DIFaddFL{0.5ex}] \noalign{\vskip 0.5ex}
  \DIFaddFL{$\chi_{Y'a}^{(XY)} = R {\left( 2 \zeta \right)}^{-1} \left[   \hat{p}_X^\female w_{A}^{\female} w_{a}^{\male} w_{Aa}^{\male} (1 - \alpha_\Delta^{\male}) \right]/ \left( \bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\male}_{D} \right)  $}\\ [\DIFaddFL{1ex}] \hline 
  \noalign{\vskip 0.5ex}
  \DIFaddFL{$m$ is a neo-W ($k=1$) }\\ [\DIFaddFL{0.5ex}] \hline \noalign{\vskip 1ex}
  \DIFaddFL{$\Lambda_{W'A}^{(XY)} = {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{A}^{\female} w_{AA}^{\female}+(1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female}(1+\alpha_{\Delta}^{\female})\right]/ \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right) $ }\\ [\DIFaddFL{0.5ex}] \noalign{\vskip 0.5ex}
  \DIFaddFL{$\Lambda_{W'a}^{(XY)} = {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{a}^{\female} w_{aa}^{\female}+\bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female}(1-\alpha_{\Delta}^{\female})\right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right) $ }\\ [\DIFaddFL{0.5ex}] \noalign{\vskip 0.5ex}
  \DIFaddFL{$\chi_{W'A}^{(XY)} = R {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ (1-\bar{p}^{\male}) w_{a}^{\male} w_{A}^{\female} w_{Aa}^{\female} (1+\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right)  $}\\ [\DIFaddFL{0.5ex}] \noalign{\vskip 0.5ex}
  \DIFaddFL{$\chi_{W'a}^{(XY)} = R {\left[ 2 (1 - \zeta) \right]}^{-1} \left[ \bar{p}^{\male} w_{A}^{\male} w_{a}^{\female} w_{Aa}^{\female} (1-\alpha_{\Delta}^{\female}) \right] / \left(\bar{w}_H^\female \bar{w}_H^\male \bar{w}^{\female}_{D} \right) $ }\\ [\DIFaddFL{1ex}]
  \hline \hline 
   \end{tabular}
   \begin{flushleft} 
\DIFaddFL{$\bar{p}^{\male}=(1-\hat{q}_{Y})\hat{p}_X^\male + \hat{q}_{Y}\hat{p}_Y^\male$ is the average frequency of the $A$ allele among X- and Y-bearing male gametes.
$\hat{q}_{Y}$ is the frequency of Y-bearing male gametes. 
%DIF >       \item $R$ is the probability of recombination between loci $\mathbf{A}$ and $\mathbf{M}$.
$\zeta$ is the zygotic sex ratio (fraction male).
$\bar{w}^{\circ}_{D}$ is the mean fitness of diploids of sex $\circ$ $\in\{\female,\male\}$}\DIFaddendFL .
\DIFaddbeginFL \DIFaddFL{$\bar{w}_H^\circ$ is the mean fitness of haploids from sex $\circ$, see }\nameref{tab:meanfitnesses}\DIFaddFL{.
...
\DIFdelbegin \DIFdel{We can draw a number of key }\DIFdelend %DIF > The new sex-determining allele increases in frequency when rare when the largest eigenvalue is greater than one ($\lambda_m^{(XY)}>1$). 
%DIF > If the average change in frequency of the two haplotypes that carry the $m$ allele ($Am$ and $am$) is positive, invasion will always occur, i.e., if $(\Lambda_{mA} + \Lambda_{ma})/2 > 1$ then $\Lambda > 1$. 
%DIF > The leading eigenvalue is the largest solution of $f(\lambda_m^{(i)})=(\lambda_m^{(i)})^2+b\lambda_m^{(i)}+c=0$ and the Perron-Frobenius theorem guarantees that it is positive, unique, and real.  
%DIF > Since $f(\Lambda_{mA}^{(i)})$ and $f(\Lambda_{ma}^{(i)})$ are of opposite signs, the leading eigenvalue must fall between these two quantities and is the larger of them when $R = 0$ (see \nameref{file:Mathematica}).  
...
\DIFadd{The haplotypic growth rates and dissociative forces are listed in Table \ref{tab:haplotype_growth} for a neo-Y and neo-W invading an ancestrally XY system.
From this table and the arguments above we draw four main }\DIFaddend points about the \DIFaddbegin \DIFadd{generic }\DIFaddend invasion of neo-Y and neo-W mutations \DIFdelbegin \DIFdel{from Table \ref{tab:haplotype_growth}. 
First, Fisherian sex ratio }\DIFdelend \DIFaddbegin \DIFadd{(without specifying the ancestral equilibrium):
(1) Fisherian sex-ratio }\DIFaddend selection will favour the spread of a \DIFdelbegin \DIFdel{neo-Y }\DIFdelend \DIFaddbegin \DIFadd{neo-W }\DIFaddend and inhibit the spread of a \DIFdelbegin \DIFdel{neo-W }\DIFdelend \DIFaddbegin \DIFadd{neo-Y }\DIFaddend if the ancestral zygotic sex ratio is biased towards \DIFdelbegin \DIFdel{females, $\zeta<1/2$ }\DIFdelend \DIFaddbegin \DIFadd{males }\DIFaddend (i.e., the first factor of the $\Lambda_{mi}^{(XY)}$ is greater than one for a \DIFdelbegin \DIFdel{neo-Y }\DIFdelend \DIFaddbegin \DIFadd{neo-W }\DIFaddend and less than one for a \DIFdelbegin \DIFdel{neo-W).
...
Second}\DIFdelend \DIFaddbegin \DIFadd{neo-Y when $\zeta>1/2$).
...
Importantly}\DIFaddend , invasion by a neo-Y (neo-W) does not directly depend on the fitness of female (male) diploids. 
...
\DIFdelbegin \DIFdel{Finally, invasions by a neo-Y and a }\DIFdelend %DIF > Beyond the direct effects of selection, there are also indirect effects caused by past selection molding the equilibrium allele frequencies ($\hat{p}^\circ_i$ terms), which generally do depend on selection in both sexes. 
\DIFaddbegin \DIFadd{(3) Haploid selection thus plays two roles, generating Fisherian selection to equalize the ancestral sex-ratio (through $\zeta$) and generating selection for the neo-Y/}\DIFaddend neo-W \DIFaddbegin \DIFadd{through associations with haploid selected loci, which can distort the sex ratio. 
Each role influences the invasion dynamics of a new sex-determining allele, allowing the sex ratio to become more or less biased during a transition (as previously found in two special cases; \mbox{%DIFAUXCMD
\cite{Kozielska:2010vm,Ubeda:2015fx}}%DIFAUXCMD
). 
(4) Finally, Table \ref{tab:haplotype_growth} shows that the genetic contexts that arise during cis- and trans-GSD transitions }\DIFaddend are qualitatively different.
This is because\DIFaddbegin \DIFadd{, in an ancestrally XY system, }\DIFaddend a gamete with the neo-Y always pairs with a female gamete containing an X, Fig \ref{fig:model_outline}C.
By contrast, a gamete with a neo-W can pair with an X- or Y-bearing male gamete, Fig \ref{fig:model_outline}D.
Consequently, neo-W-bearing females obtain a different frequency of $A$ alleles from mating \DIFdelbegin \DIFdel{(when $\hat{p}_X^\male \neq \hat{p}_Y^\male$) }\DIFdelend compared to ancestral ($MM$) females \DIFaddbegin \DIFadd{($\bar{p}^\male$ versus $\hat{p}_X^\male$, respectively). 
This can inhibit or favour the spread of a neo-W}\DIFaddend .

In order to explicitly determine the conditions under which a \DIFdelbegin \DIFdel{rare }\DIFdelend new sex-determining allele spreads, we \DIFdelbegin \DIFdel{must }\DIFdelend \DIFaddbegin \DIFadd{next }\DIFaddend calculate the equilibrium frequency of the $A$ allele (i.e., $\hat{p}^\female_X$, $\hat{p}^\male_X$, and $\hat{p}^\male_Y$) and Y-bearing male gametes (\DIFdelbegin \DIFdel{$\hat{q}$}\DIFdelend \DIFaddbegin \DIFadd{$\hat{q}_{Y}$}\DIFaddend ) in the ancestral population. 
...
Polymorphisms can be maintained by mutation-selection balance or \DIFdelbegin \DIFdel{transiently present }\DIFdelend \DIFaddbegin \DIFadd{occur transiently }\DIFaddend during the spread of beneficial alleles. 
\DIFdelbegin \DIFdel{However, polymorphisms maintained by selection can maintain alleles at intermediate allele frequencies for longer periods. 
Here, }\DIFdelend %DIF > However, polymorphisms maintained by selection can maintain alleles at intermediate allele frequencies for longer periods. 
\DIFaddbegin \DIFadd{Here, however, }\DIFaddend we focus on polymorphisms maintained by selection \DIFdelbegin \DIFdel{, where the $A$ allele reaches a stable intermediate equilibrium frequency under the ancestral sex-determining system before the new sex-determining allele ($m$) arises. 
}\DIFdelend \DIFaddbegin \DIFadd{for longer periods. 
...
}\DIFaddend Such polymorphisms can be maintained by heterozygote advantage, sexually-antagonistic selection, ploidally-antagonistic selection, or a combination~\cite{Immler:2012tl}.
...
\DIFaddend When there is complete linkage between the ancestral sex-determining locus and the \DIFdelbegin \DIFdel{$\mathbf{A}$ locus }\DIFdelend \DIFaddbegin \DIFadd{selected locus $\mathbf{A}$ }\DIFaddend ($r=0$), either the $A$ allele or the $a$ allele must be fixed in gametes containing a Y allele \DIFaddbegin \DIFadd{(}\nameref{app:eq_stab}\DIFadd{)}\DIFaddend . 
...
Neo-W alleles, on the other hand, can invade an ancestral XY system \DIFdelbegin \DIFdel{under some conditions }\DIFdelend (the full invasion conditions are given in \nameref{app:inv_cond}; equations \ref{Binvasion} and \ref{Ainvasion}).
\DIFdelbegin \DIFdel{Counterintuitively, selection on loci within the non-recombining region around the sex-determining locus can favour the invasion of a less closely linked }\DIFdelend \DIFaddbegin \DIFadd{Invasion occurs when }\DIFaddend neo-W \DIFdelbegin \DIFdel{, whatever the form of selection maintaining }\DIFdelend \DIFaddbegin \DIFadd{females can have higher fitness than the XX females in the ancestral population.
Neo-W invasion is possible under all forms of selection that can maintain }\DIFaddend a polymorphism (sexually-antagonistic selection, overdominance, ploidally-antagonistic selection, or some combination, \DIFdelbegin \DIFdel{Fig \ref{fig:SexAntagTighter}}\DIFdelend \DIFaddbegin \DIFadd{e.g.}\DIFaddend , \nameref{fig:temporalOverdominance}, \DIFdelbegin %DIFDELCMD < \nameref{fig:regionPloidAntag}%%%
\DIFdel{, and }%DIFDELCMD < \nameref{fig:SexAntagTighterMaleDrive}%%%
\DIFdel{). The conditions become more restrictive, 
however, with increasing recombination ($R$) between the new sex-determining locus and the selected locus.  
The invasion of completely unlinked neo-W alleles ($R = 1/2$) can occurwith overdominance in males or with haploid selection but is not possible with only sexually-antagonistic selection if selection is directional in each diploid sex (see }%DIFDELCMD < \nameref{file:Mathematica}%%%
\DIFdel{).
To develop an intuition for how less closely linked neo-W alleles invade ($R>r$), we here focus }\DIFdelend \DIFaddbegin \nameref{fig:SexAntagTighterMaleDrive}\DIFadd{, and }\nameref{fig:regionPloidAntag}\DIFadd{). Thus, 
}\vspace{0.5cm}

\noindent\textit{\DIFadd{Conclusion 1:}}
...
\textbf{\DIFadd{Selection on loci in or near the non-recombining region around the ancestral sex-determining locus ($r\approx0$) prevents cis-GSD transitions (XY $\leftrightarrow$ XY, ZW $\leftrightarrow$ ZW) but can spur trans-GSD transitions (XY $\leftrightarrow$ ZW).}}
...
\noindent 
\DIFadd{To clarify conditions under which trans-GSD transitions can occur, we focus here }\DIFaddend on cases where there is no haploid selection \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{(and hence no sex-ratio bias) and }\DIFaddend discuss the additional effect of haploid selection in \nameref{app:inv_cond}. 
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF < Tighter linkage allows looser sex-linkage to evolve, even in absence of haploid selection
%DIF < %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
{%DIFAUXCMD
%DIFDELCMD < {\bf %%%
\DIFdelFL{Transitions between XY and ZW systems can occur even when the new sex-determining locus is less tightly linked to a locus under sexually-antagonistic selection (even without haploid selection).}%DIFDELCMD < }
%DIFDELCMD < %%%
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:SexAntagTighter}
...
%DIFDELCMD < %%%
\DIFdel{If we categorise the $a$ allele as being ancestrally `male-beneficial' via the fact that it is fixed on Y backgrounds, then $\Lambda_{W'A}^{(XY)}>1$ indicates that the }\DIFdelend \DIFaddbegin \DIFadd{Broadly, it is possible for }\DIFaddend neo-W \DIFdelbegin \DIFdel{spreads when found with the ancestrally `female-beneficial' allele.
...
When the $a$ allele is strongly favoured on the X background }\DIFdelend \DIFaddbegin \DIFadd{females to have higher fitness than XX females for two reasons.
Firstly, because the ancestral X experiences selection in both males and females, the X may be unable to specialize strongly on an allele favoured }\DIFaddend in \DIFaddbegin \DIFadd{females.
Secondly, an allele can be associated with the Y and yet favoured in females.
In turn, a neo-W can spread because (a) it is only found in females and is therefore unleashed from counterselection in }\DIFaddend males (\DIFdelbegin \DIFdel{$w_{aa}^\male$ sufficiently large relative to $w_{Aa}^\male$), }\DIFdelend \DIFaddbegin \DIFadd{corresponding to $\Lambda_{W'A}^{(XY)}>1$), (b) it allows Y-associated alleles into females (corresponding to $\Lambda_{W'a}^{(XY)}>1$).
}

\DIFadd{We first give an example where }\DIFaddend neo-W-$A$ haplotypes can spread \DIFdelbegin \DIFdel{($\Lambda_{W'A}^{(XY)}>1$, see grey region in Fig \ref{fig:regionplots}A) because they produce higher fitness females ($AA$ or $Aa$ genotypes) and are }\DIFdelend \DIFaddbegin \DIFadd{because the neo-W is }\DIFaddend unleashed from counterselection in males \DIFaddbegin \DIFadd{(case (a), where $\Lambda_{W'A}^{(XY)}>1$).
...
In this case, polymorphism on the ancestral-X indicates suboptimal specialisation for females fitness, which occurs because the $A$ allele is counterselected in males (requires that $w^\male_{Aa}$  sufficiently small relative to $w^\male_{aa}$)}\DIFaddend . 
\DIFdelbegin %DIFDELCMD < 
...
%DIFDELCMD < \centering
%DIFDELCMD < %%%
%DIF < \centerline{\includegraphics[width=1.25\linewidth]{Region_plot_combined_Mike}}
%DIF < \centerline{\includegraphics[width=1.25\linewidth]{Region_plot_combined}}
%DIFDELCMD < \caption{%
{%DIFAUXCMD
%DIFDELCMD < {\bf %%%
\DIFdelFL{When the ancestral XY locus is tightly linked to a locus under selection ($r=0$), one or both neo-W haplotypes can spread. }%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelFL{We vary the fitness of male homozygotes relative to heterozygotes ($w_{Aa}^\Hermaphrodite=1$) and only consider stable equilibria at which both $\mathbf{A}$ locus alleles are maintained and the $a$ allele is initially fixed on Y backgrounds, region outlined. 
...
}%DIFDELCMD < \nameref{fig:positionOverdominance} %%%
\DIFdelFL{shows the dynamics arising with the parameters marked with a dagger. 
Here, there is no haploid selection $t^\Hermaphrodite = \alpha^\Hermaphrodite_\Delta = 0$.
}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:regionplots}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{When only one neo-W haplotype has growth rate greater than one (see Fig \ref{fig:regionplots}), a neo-W allele can invade as long as Eq }%DIFDELCMD < \eqref{eq:lambdasGen} %%%
\DIFdel{is satisfied, which may require that the recombination rate, $R$, is small enough. 
...
For example, tightly sex-linked loci that experience sexually-antagonistic selection can drive trans-GSD transitions in which the new sex-determining locus is less closely linked to the locus under selection (Fig \ref{fig:SexAntagTighter}}\DIFdelend \DIFaddbegin \DIFadd{Neo-Ws, however, spend no time in males and can build stronger associations with the female-beneficial $A$ allele, allowing them to spread (see gray region in Fig \ref{fig:regionplots}A}\DIFaddend ).

\DIFdelbegin \DIFdel{Given that the $a$ allele can be considered ancestrally `male-beneficial' because it is fixed on Y backgrounds, it is surprising that }\DIFdelend \DIFaddbegin \DIFadd{We next give an example where }\DIFaddend neo-W-$a$ haplotypes can \DIFdelbegin \DIFdel{sometimes be favoured by selection in females ($\Lambda_{W'a}^{(XY)}>1$). 
...
If }\DIFdelend \DIFaddbegin \DIFadd{spread because they bring in female beneficial alleles associated with the Y (case (b), where $\Lambda_{W'a}^{(XY)}>1$). 
When }\DIFaddend there is overdominance in males, X-$A$ Y-$a$ males have high fitness and the $A$ allele is favoured by selection on the X background in males.
Therefore, the $A$ allele can be polymorphic or even fixed on the X background despite selection favouring the $a$ allele in females (e.g., see \DIFdelbegin \DIFdel{outlined }\DIFdelend \DIFaddbegin \DIFadd{non-hatched }\DIFaddend region in Fig \ref{fig:regionplots}B and~\cite{Lloyd1977,Otto2014}). 
In such cases, neo-W-$a$ haplotypes can spread because they create more $Aa$ and $aa$ females when pairing with an X-bearing gamete from males and because they bring \DIFaddbegin \DIFadd{more of the }\DIFaddend Y-$a$ \DIFdelbegin \DIFdel{haplotypes }\DIFdelend \DIFaddbegin \DIFadd{haplotype }\DIFaddend into females, where it has higher fitness (Fig \ref{fig:model_outline}D). 
...
\centering
%DIF > \centerline{\includegraphics[width=1.25\linewidth]{Region_plot_combined_Mike}}
\centerline{\includegraphics[width=1.4\linewidth, right]{../Plots/Region_plot_combined.eps}}
\caption{
{\bf \DIFaddFL{When the ancestral XY locus is tightly linked to a locus under selection ($r=0$), one or both neo-W haplotypes can spread (no haploid selection).}}
\DIFaddFL{We vary the fitness of male homozygotes relative to heterozygotes ($w_{Aa}^\circ=1$) and only consider stable equilibria at which both $\mathbf{A}$ locus alleles are maintained and the $a$ allele is initially fixed on the Y (non-hatched region). 
Here, selection in females can favour the $A$ allele (panel A, $w_{aa}^\female=0.85$, $w_{AA}^\female=1.05$), favour the $a$ allele (panel B, $w_{aa}^\female=1.05$, $w_{AA}^\female=0.85$), or be overdominant (panel C, $w_{aa}^\female=w_{AA}^\female=0.6$). 
...
}\nameref{fig:positionOverdominance} \DIFaddFL{shows the dynamics arising with the parameters marked with a dagger. 
%DIF > Here, there is no haploid selection $t^\circ = \alpha^\circ_\Delta = 0$.
}}
\label{fig:regionplots}
\end{figure}

\DIFaddend In some cases, both \DIFdelbegin \DIFdel{W-}\DIFdelend \DIFaddbegin \DIFadd{neo-W-}\DIFaddend $A$ and \DIFdelbegin \DIFdel{W-}\DIFdelend \DIFaddbegin \DIFadd{neo-W-}\DIFaddend $a$ haplotypes can spread.
...
\DIFdelbegin \DIFdel{Wherever }\DIFdelend \DIFaddbegin \DIFadd{Whenever }\DIFaddend both haplotypic growth rates are greater than one, invasion by a neo-W is expected regardless of its linkage with the selected locus (i.e., for any $R$\DIFaddbegin \DIFadd{)}\DIFaddend , see \nameref{fig:positionOverdominance} and \nameref{fig:temporalOverdominance} for examples\DIFdelbegin \DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{. 
As a consequence, evolution can favor a new sex determination system on a different chromosome ($R = 1/2$), despite the fact that this unlinks the sex-determining locus from the selected locus}\DIFaddend .

\DIFaddbegin \DIFadd{When only one neo-W haplotype has growth rate greater than one (see Fig \ref{fig:regionplots}), a neo-W allele can invade as long as Eq }\eqref{eq:lambdasGen} \DIFadd{is satisfied, which may require that the recombination rate, $R$, is small enough.
Nevertheless, because we assume here that $r$ is small, these results indicate that a more loosely linked sex-determining region ($r<R$) can spread.
For example, tightly sex-linked loci that experience sexually-antagonistic selection can drive trans-GSD transitions in which the new sex-determining locus is less closely linked ($R>r$, Fig \ref{fig:SexAntagTighter}), but the analysis in }\nameref{file:Mathematica} \DIFadd{indicates that a new unlinked sex-determining allele ($R=1/2$) cannot invade when selection is purely sexually-antognistic (directional selection in each sex and no haploid selection).
}
...
\begin{figure}[!h]
\centering
\centerline{\includegraphics[width=1.4\linewidth, right]{../Plots/PositionPlot_SexAntagTighter.eps}}
\caption{
{\bf \DIFaddFL{Transitions between XY and ZW systems can occur even when the new sex-determining locus is less tightly linked to a locus under sexually-antagonistic selection (no haploid selection).}}
\DIFaddFL{In panel A, linkage is initially tight relative to selection and a neo-W can invade even when it is less tightly linked with the selected locus ($r<R$; unshaded region around }\textbf{\DIFaddFL{*}}\DIFaddFL{).
...
A very loosely linked neo-W does not spread in this case (blue and green lines overlap and go to 0 in inset). 
Fitness parameters are: $w_{AA}^\female = 1.05$, $w_{aa}^\male = 1.2$, $w_{aa}^\female = w_{AA}^\male = 0.85$, $w_{Aa}^\circ = 1$.%DIF > ,  $t^\circ = \alpha^\circ_\Delta = 0$.
%DIF > \textcolor{red}{consider removing panel A, which is repeated in Fig 3.}
%DIF > \textcolor{blue}{probably better to remove from panel 3, eh?}
}}
...
\DIFaddend Assuming selection is weak relative to recombination, van Doorn and Kirkpatrick~\cite{vanDoorn:2010hu} showed that invasion by a neo-W allele occurs under the same conditions as \DIFdelbegin \DIFdel{`pseudo-fixation' (at pseudo-fixation the neo-W reaches its maximum frequency among eggs, which is usually 1/2, but can deviate from 1/2 when there is haploid selection before censusing).
...
\DIFaddbegin \DIFadd{From the arguments above we reach:
}\vspace{0.5cm}

\noindent\textit{\DIFadd{Conclusion 2:}}
\textbf{
\DIFadd{With tight linkage between a selected locus and the ancestral sex-determining locus ($r\approx0$), trans-GSD transitions (XY $\leftrightarrow$ ZW) can be favoured by selection }\textit{\DIFadd{even if they weaken sex-linkage}} \DIFadd{($r<R$), potentially shifting sex determination to a different chromosome ($R=1/2$). 
Such transitions can also lead to the maintenance of multifactorial sex-determination systems.
}}
\vspace{0.5cm} 

\noindent
\DIFadd{With haploid selection, Conclusions 1 \& 2 continue to apply (}\nameref{app:inv_cond}\DIFadd{). 
The parameters for which neo-W-$A$ and neo-W-$a$ haplotypes spread under various forms of haploid selection are plotted in }\nameref{fig:regionMaleDrive}\DIFadd{, }\nameref{fig:regionMaleGS}\DIFadd{, }\nameref{fig:regionFemaleDrive}\DIFadd{, }\nameref{fig:regionFemaleGS}\DIFadd{.
In particular, we note that adding haploid selection allows shifts in sex determination to a different chromosome ($R=1/2$) even when selection is sexually antagonistic with directional selection in each diploid sex, e.g., }\nameref{fig:SexAntagTighterMaleDrive}\DIFadd{.  
Furthermore, haploid selection allows variation to be maintained by ploidally-antagonistic selection, under which trans-GSD transitions may also be favoured, }\nameref{fig:regionPloidAntag}\DIFadd{.
Some cases of XY $\rightarrow$ ZW transitions where $r=0$, $R=1/2$, and selection is ploidally-antagonistic (meiotic drive in males opposed by diploid selection) were studied by Kozielska et al. \mbox{%DIFAUXCMD
\cite{Kozielska:2010vm}}%DIFAUXCMD
, who found that sex-ratio biases are reduced during these transitions. 
...
\begin{equation}
\lambda_{Y'}^{(XY)} = 1 + \frac{1}{4}\DIFdelbegin \DIFdel{V_{A}}\DIFdelend \DIFaddbegin \bar{p}\DIFadd{(1-}\bar{p}\DIFadd{)}{\DIFaddend S_{A}\DIFaddbegin }\DIFaddend ^2\frac{ \left( r-R \right) }{r R}+O\left(\epsilon^3 \right) 
...
\noindent
where \DIFdelbegin \DIFdel{$V_{A}=\bar{p}(1-\bar{p})$ is the variance in the equilibrium }\DIFdelend \DIFaddbegin \DIFadd{$\bar{p}$ is the }\DIFaddend frequency of $A$\DIFdelbegin \DIFdel{and $S_{A}=(D^\male +\alpha_{\Delta}^\male+t^\male) - (D^\female+\alpha_{\Delta}^\female+t^\female)$ }\DIFdelend \DIFaddbegin \DIFadd{, to leading-order (Eq \ref{eq:pAve}), and $S_{A}=(\bar{s}^\male +\alpha_{\Delta}^\male+t^\male) - (\bar{s}^\female+\alpha_{\Delta}^\female+t^\female)$ }\DIFaddend describes sex differences in selection for the $A$ versus $a$ allele across diploid selection, meiosis, and gametic competition.
The diploid selection term, \DIFdelbegin \DIFdel{$D^\Hermaphrodite=\big{[} \bar{p}s^\Hermaphrodite+(1-\bar{p})h^\Hermaphrodite s^\Hermaphrodite\big{]} -\big{[} \bar{p}h^\Hermaphrodite s^\Hermaphrodite+(1-\bar{p}) \big{]}$}\DIFdelend \DIFaddbegin \DIFadd{$\bar{s}^\circ=\big{[} \bar{p}s^\circ+(1-\bar{p})h^\circ s^\circ\big{]} -\big{[} \bar{p}h^\circ s^\circ+(1-\bar{p}) \big{]}$}\DIFaddend , is the difference in fitness between $A$ and $a$ alleles in diploids of sex \DIFdelbegin \DIFdel{$\Hermaphrodite \in \{\female,\male\}$, where $\bar{p}$ is the leading-order probability of mating with an $A$-bearing gamete from the opposite sex (equation \ref{eq:pAve})}\DIFdelend \DIFaddbegin \DIFadd{$\circ \in \{\female,\male\}$}\DIFaddend . 
The difference in $A$-allele-frequency among Y-bearing sperm versus X-bearing sperm is\DIFdelbegin \DIFdel{given by $\hat{p}^\male_Y-\hat{p}^\male_X=V_{A}\big{(}D^\male - D^\female+\alpha_{\Delta}^\male-\alpha_{\Delta}^\female+t^\male-t^\female\big{)}(1-2r)/(2r)$}\DIFdelend \DIFaddbegin \DIFadd{, at equilibrium, $\hat{p}^\male_Y-\hat{p}^\male_X=\bar{p}(1-\bar{p})S_{A}(1-2r)/(2r)$}\DIFaddend . 

%The new sex-determining allele, $m$, will spread in an XY system if $\lambda_{m}^{(XY)}>1$. 
Eq \eqref{eq:lambda_neoY} demonstrates that, under weak selection, a neo-Y allele will invade an XY system ($\lambda_{Y'}^{(XY)}>1$) if and only if it is more closely linked to the selected locus than the ancestral sex-determining locus (i.e., if $R<r$\DIFdelbegin \DIFdel{; note that $V_{A}S_{A}^2$ is strictly positive as long as $\mathbf{A}$ is polymorphic). }\DIFdelend \DIFaddbegin \DIFadd{). %DIF > ; note that $\bar{p}(1-\bar{p})S_{A}^2$ is strictly positive as long as $\mathbf{A}$ is polymorphic). 
}\DIFaddend This echoes our results above where a neo-Y could never invade if $r\approx0$. 
...
\DIFdelbegin \DIFdel{With weak selection and no haploid selection ($t^\Hermaphrodite=\alpha^\Hermaphrodite_{\Delta}=0$), the spread of a neo-W is equivalent to the spread of a neo-Y ($\lambda_{W'}^{(XY)}=\lambda_{Y'}^{(XY)}$), such }\DIFdelend \DIFaddbegin \vspace{0.5cm}
\noindent\textit{\DIFadd{Conclusion 3A:}}\DIFadd{\
}\textbf{\DIFadd{New sex-determining alleles (causing cis-GSD transitions, XY $\leftrightarrow$ XY or ZW $\leftrightarrow$ ZW) are favoured if they arise more closely linked with a locus that experiences (haploid and/or diploid) selection than the ancestral-sex-determining locus ($R<r$).}}
\vspace{0.5cm}
...
\DIFadd{Similarly, in the absence of haploid selection ($t^\circ=\alpha^\circ_{\Delta}=0$), Eq }\eqref{eq:lambda_neoW} \DIFadd{indicates }\DIFaddend that trans-GSD transitions can \DIFdelbegin \DIFdel{also occur }\DIFdelend \DIFaddbegin \DIFadd{occur if and }\DIFaddend only if the new sex-determining locus is more closely linked to a locus under selection\DIFdelbegin \DIFdel{(}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $R<r$\DIFdelbegin \DIFdel{)}\DIFdelend , as found by \cite{vanDoorn:2010hu}. 
\DIFdelbegin \DIFdel{When there is }\DIFdelend \DIFaddbegin \DIFadd{With }\DIFaddend haploid selection, \DIFdelbegin \DIFdel{invasion also typically occurs when the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend neo-W \DIFaddbegin \DIFadd{is also usually favoured when it }\DIFaddend is more closely linked to the selected locus than the ancestral sex-determining region \DIFdelbegin \DIFdel{(Fig \ref{fig:Combination_Centimorgans})}\DIFdelend \DIFaddbegin \DIFadd{is, ($R<r$, e.g., Figs \ref{fig:SexAntagTighter}B and \ref{fig:Combination_Centimorgans}); this is true unless the last term in Eq }\eqref{eq:lambda_neoW} \DIFadd{is negative and dominant over the first, which requires relatively restrictive combinations of selection and recombination parameters}\DIFaddend . 
For example, \DIFdelbegin \DIFdel{if the $\mathbf{A}$ locus is unlinked to the ancestral sex-determining locus ($r=1/2$), a more closely linked }\DIFdelend \DIFaddbegin \DIFadd{with haploid selection, a }\DIFaddend neo-W \DIFaddbegin \DIFadd{will always be favoured if it arises in linkage with a selected locus }\DIFaddend ($R<1/2$) \DIFdelbegin \DIFdel{can always invade.
In this case, there is no ancestral association between $\mathbf{A}$ alleles and sex chromosomes in males, $\left( \hat{p}^\male_Y-\hat{p}^\male_X \right)=0$, see Eq }%DIFDELCMD < \eqref{eq:freq_diffs}%%%
\DIFdel{. 
...
}\DIFdelend \DIFaddbegin \DIFadd{that is ancestrally autosomal ($r=1/2$, leading to $ \hat{p}^\male_Y-\hat{p}^\male_X =0$). 
}\DIFaddend 
...
\noindent\textit{\DIFadd{Conclusion 3B:}}\DIFadd{\
}\textbf{\DIFadd{New sex-determining alleles (causing trans-GSD transitions, XY $\leftrightarrow$ ZW) are usually favoured if they arise more closely linked with a locus that experiences (haploid and/or diploid) selection than the ancestral-sex-determining locus ($R<r$).
}}
\vspace{0.5cm}
\DIFaddend 

\DIFaddbegin \noindent
\DIFaddend However, with haploid selection and \DIFaddbegin \DIFadd{some ancestral sex-linkage (}\DIFaddend $r<1/2$\DIFdelbegin \DIFdel{, the additional term in }\DIFdelend \DIFaddbegin \DIFadd{; allowing allele frequency differences on the X and Y), the term in square brackets in }\DIFaddend Eq \eqref{eq:lambda_neoW} can be positive.
This \DIFdelbegin \DIFdel{can allow neo-W invasion ($\lambda_{W'}^{(XY)}>1$), even when the new sex-determining locus is less closely linked to 
the selected locus ($R>r$). 
%DIF < These transitions are unusual because, when $R>r$, associations that selection has built up between alleles more favourable in one sex and alleles that determine sex will be weakened. 
%DIF < Mean diploid fitness therefore decreases during heterogametic transitions that create looser sex-linkage (Fig \ref{fig:Combination_MeanFit}B,D). 
%DIF < Eq \eqref{eq:lambda_neoW} shows that, with weak selection, neo-W alleles can invade an XY system for a large number of selective regimes. 
%DIF < To clarify the parameter space under which $\lambda_{W'}^{(XY)}>1$ , we consider several special cases. 
%DIF < Firstly, if the $\mathbf{A}$ locus is unlinked to the ancestral sex-determining region ($r=1/2$), a more closely linked neo-W ($R<1/2$) can always invade because there is no ancestral association between $A$ alleles and sex chromosomes in males, $\left( \hat{p}^\male_Y-\hat{p}^\male_X \right)=0$, see Eq \eqref{eq:freq_diffs}. 
...
%DIF < Indeed, invasion typically occurs when the neo-W is more closely linked to the selected locus than the ancestral sex-determining region (Fig \ref{fig:Combination_Centimorgans}).
}\DIFdelend \DIFaddbegin \DIFadd{leads to 
}

\vspace{0.5cm}
\noindent\textit{\DIFadd{Conclusion 3C:}}\DIFadd{\
}\textbf{\DIFadd{With haploid selection, new sex-determining alleles (causing trans-GSD transitions, XY $\leftrightarrow$ ZW) can spread even if they arise less closely linked with a locus that experiences selection than the ancestral-sex-determining locus ($r<R$).}}
\vspace{0.5cm}
...
\DIFaddend To clarify the parameter space under which \DIFdelbegin \DIFdel{invasion occurs despite looser sex-linkage ($\lambda_{W'}^{(XY)}>1$ despite }\DIFdelend \DIFaddbegin \DIFadd{neo-W alleles spread despite looser linkage with the selected locus (}\DIFaddend $R>r$), we focus on \DIFdelbegin \DIFdel{the special case where $R=1/2$ and $r<1/2$ (e.g., the selected locus is on the ancestral sex chromosome and the novel sex-determining locus arises on an autosome). 
...
\cite{Kozielska:2010vm}}%DIFAUXCMD
. 
Consider, for example, the case where the $\mathbf{A}$ locus is linked to the ancestral sex-determining locus ($r<1/2$) and experiences }\DIFdelend \DIFaddbegin \DIFadd{We can also compare transitions in genetic sex-determination where sex-ratio bias increases, decreases, or remains equal. 
...
In this case, }\DIFdelend the zygotic sex ratio \DIFdelbegin \DIFdel{can be }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend initially biased only \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{when }\DIFaddend the ancestral sex-determining system is XY (\DIFdelbegin \DIFdel{Fig \ref{fig:model_outline}A and Fig }\DIFdelend \DIFaddbegin \DIFadd{Figs \ref{fig:model_outline}B and \ref{fig:SexRatioBad}A) and not ZW (Figs \ref{fig:model_outline}B and }\DIFaddend \ref{fig:SexRatioBad}B).
...
\textbf{\DIFadd{When selection is weak relative to recombination, the presence of haploid selection equally favors the spread of new sex determination systems that reduce sex-ratio bias (benefiting from Fisherian sex ratio selection) or that generate a sex-ratio bias (benefiting from associations with selected alleles). }} 
\vspace{0.5cm}
...
When $R<1/2$, the invading neo-Y becomes associated with the male meiotic drive allele and the zygotic sex ratio \DIFdelbegin \DIFdel{actually }\DIFdelend evolves to become male-biased \DIFaddbegin \DIFadd{(as occurs in \mbox{%DIFAUXCMD
\cite{Ubeda:2015fx}}%DIFAUXCMD
...
}\DIFdelend \DIFaddbegin \DIFadd{compared to ZZ males, which bear a high frequency of the driven allele, $a$, from their mothers.
}\DIFaddend 
...
%DIFDELCMD < \lambda_{ESD'}^{(XY)} =& 1 + \frac{{(1-2k)}^2}{4}V_{A}{S_{A}}^2\frac{r-R}{r R} \\
%DIFDELCMD < &+\frac{k(\hat{p}^\male_Y-\hat{p}^\male_X)}{2}\left[ k \left(2\alpha_{\Delta}^\male-2\alpha_{\Delta}^\female+t^\male-t^\female \right) -2(1-k)S_{A}\right]+O\left(\epsilon^3\right).
%DIFDELCMD < \end{split}
%DIFDELCMD < %%%
...
That is, \DIFdelbegin \DIFdel{recombination between the selected locus and the novel sex-determining locus, $R$, does not enter into the }\DIFdelend \DIFaddbegin \DIFadd{ESD with }\DIFaddend $k=1/2$ \DIFdelbegin \DIFdel{results}\DIFdelend \DIFaddbegin \DIFadd{behaves as if the $\mathbf{M}$ and $\mathbf{A}$ loci were unlinked, regardless of the actual value of $R$}\DIFaddend .
This is because sex is \DIFdelbegin \DIFdel{essentially }\DIFdelend randomized each generation \DIFdelbegin \DIFdel{, %DIF < as if the novel sex-determiner were on an unlinked site that half the time acts as a neo-W and half the time as a neo-Y.
}\DIFdelend \DIFaddbegin \DIFadd{in individuals bearing the $m$ allele, }\DIFaddend preventing associations from building up between \DIFaddbegin \DIFadd{it and }\DIFaddend alleles at locus $\mathbf{A}$\DIFdelbegin \DIFdel{and sex}\DIFdelend . 
...
As discussed above, $\lambda_{Y'\rvert R=1/2}^{(XY)}$ is necessarily less than \DIFdelbegin \DIFdel{one}\DIFdelend \DIFaddbegin \DIFadd{or equal to one when selection is weak (Conclusion 3A)}\DIFaddend , but $\lambda_{W'\rvert R=1/2}^{(XY)}$ can be greater than one if there is haploid selection \DIFaddbegin \DIFadd{(see Conclusion 3C)}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{(likewise, ESD invades a ZW system for the same reasons that a neo-Y can).
...
%DIFAUXCMD
\DIFdelend \DIFaddbegin \noindent\textit{\DIFadd{Conclusion 5:}}
\textbf{
...
\DIFadd{New sex-determination systems are typically expected to spread when they equalise the sex ratio and}\DIFaddend /\DIFaddbegin \DIFadd{or when they increase linkage with loci that experience sex-differences in selection~\mbox{%DIFAUXCMD
\cite{Blaser2012, vanDoorn2014re}}%DIFAUXCMD
.
In accordance with the latter mechanism, we find that sex-differences in selection at the haploid stage can favour cis- }\DIFaddend or \DIFdelbegin \DIFdel{ploidally-antagonistic selection) on loci tightly linked to the ancestral sex-determining locus can favour }\DIFdelend trans-GSD transitions \DIFdelbegin \DIFdel{(XY$\leftrightarrow$ ZW) to new sex-determining systems that are less closely linked to the selected loci (e.g., see Fig \ref{fig:SexAntagTighter}). Similarly, even when linkage is weak relative to selection, we show that %DIF < new sex-determining alleles are typically favoured if they are more closely linked to a locus under haploid selection (even in the absence of diploid sex differences), which is the only condition favouring cis-GSD transitions (XY to XY or ZW to ZW). 
%DIF < In addition, with haploid selection and weak selection, 
}\DIFdelend \DIFaddbegin \DIFadd{that tighten sex-linkage (Conclusion 3A \&3B). %DIF > , as exemplified in~\cite{Ubeda:2015fx} for ESD-GSD transitions. 
Contrary to this expectation, however, we find that }\DIFaddend trans-GSD transitions can \DIFdelbegin \DIFdel{occur where the new }\DIFdelend \DIFaddbegin \DIFadd{be favoured that loosen linkage with the }\DIFaddend sex-determining locus\DIFdelbegin \DIFdel{is less closely linked to the locus under selection if }\DIFdelend \DIFaddbegin \DIFadd{, either when linkage is initially tight (Conclusions 1 \& 2, Figs \ref{fig:regionplots} \& \ref{fig:SexAntagTighter}) or when }\DIFaddend there is haploid selection (\DIFdelbegin \DIFdel{e.g.}\DIFdelend \DIFaddbegin \DIFadd{Conclusion 3C}\DIFaddend , Figs \ref{fig:Combination_Centimorgans} \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{\& }\DIFaddend \ref{fig:SexRatioBad}). 
\DIFdelbegin %DIFDELCMD < 
...
On }\DIFdelend \DIFaddbegin \DIFadd{(Conclusions 4 \& 5, Fig \ref{fig:SexRatioBad}). 
}

%DIF > We find that the spread of new sex-determining systems cannot be simply predicted from their effect on the sex ratio. 
\DIFadd{On the }\DIFaddend one hand, \DIFdelbegin \DIFdel{sex ratio }\DIFdelend \DIFaddbegin \DIFadd{sex-ratio }\DIFaddend biases caused by haploid selection can facilitate trans-GSD transitions or transitions from genetic to environmental sex determination\DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{Kozielska:2010vm}}%DIFAUXCMD
}\DIFaddend . 
...
This male bias increases the potential for a neo-W \DIFaddbegin \DIFadd{or ESD }\DIFaddend allele to invade (Table \ref{tab:haplotype_growth}), \DIFdelbegin \DIFdel{which can equalize }\DIFdelend \DIFaddbegin \DIFadd{equalizing }\DIFaddend the sex ratio (e.g., see Fig \ref{fig:SexRatioBad}B, for related examples see~\cite{Kozielska:2010vm}).
...
%DIF < For example, where a neo-Y invades and is linked with a locus that experiences haploid selection in male gametes, the sex ratio evolves to become biased \citep[e.g., see Fig \ref{fig:SexRatioBad}A and step 1 in][]{Ubeda:2015fx}.
%DIF < For example, in an ancestral ZW system, haploid selection in males can allow a linked neo-Y to invade, despite the fact that it creates a male-biased sex ratio because the neo-Y becomes associated with the driven allele (Fig \ref{fig:SexRatioBad}A).
%DIF < \textcolor{blue}{and figure 1, where the end of one panel is the same as the beginning of another, however, this figure may change).}
...
Thus, haploid selection can favour trans-GSD transitions both via sex ratio selection and via selection on alleles associated with the new sex-determining allele, }\DIFdelend \DIFaddbegin \DIFadd{(Fig \ref{fig:SexRatioBad}).
...
\DIFadd{Because both Fisherian selection to equalize the sex ratio }\DIFaddend and \DIFdelbegin \DIFdel{these selective pressures are often predicted to be of equal magnitude.
}\DIFdelend \DIFaddbegin \DIFadd{the benefits of hitchhiking with driven alleles can facilitate transitions among sex chromosome systems, we predict that haploid selection should increase the lability of sex determination systems.
...
, which can generate biased sex-ratios~\mbox{%DIFAUXCMD
\cite{Jaenike:2001vb,Burt:2006,Lloyd:1974tz,Conn:1981uw,Stehlik:2005ul,Stehlik:2006to,Field:2012fd,Field:2013cc}}%DIFAUXCMD
. 
In animals, a relatively small proportion of all genes are thought to be expressed and selected during competition in animal sperm~\mbox{%DIFAUXCMD
\cite{Zheng:2001fi,JOSEPH:2004haa,Vibranovski:2010et}}%DIFAUXCMD
.
Nevertheless, recent studies have demonstrated that sperm competition, even within a single ejaculate, can alter haploid allele frequencies and increase offspring fitness~\mbox{%DIFAUXCMD
\cite{Immler:2014im,Alavioon2017}}%DIFAUXCMD
.
Expression in the gamete is not required for haploid selection if the fitness of a gamete depends on its ability to condense DNA~\mbox{%DIFAUXCMD
\cite{Immler2018}}%DIFAUXCMD
. 
Furthermore, expression during gamete production often underlies systems of meiotic drive~\mbox{%DIFAUXCMD
\cite{Tau2007,Cocquet2012,Helleu2016}}%DIFAUXCMD
, which may be a common form of haploid selection in animals~\mbox{%DIFAUXCMD
\cite{Bachtrog2018}}%DIFAUXCMD
.
In plants, competition among gametophytes may be particularly important. 
It is estimated that 60-70\% of all genes are expressed in the male gametophyte, and these genes exhibit stronger signatures of selection than randomly-chosen genes~\mbox{%DIFAUXCMD
\cite{Borg:2009jpa,Arunkumar:2013iq,Gossmann:2014dua}}%DIFAUXCMD
. 
...
%DIF < Our results suggest that haploid selected loci that have been involved in driving turnovers between sex-determining systems could occur around the ancestral- or novel-sex-determining regions. 
%DIF < Our results suggest that polymorphic loci that are ancestrally sex-linked and under sex-specific selection could also drive heterogametic transitions between sex-determining systems. 
%DIF < As with sexually-antagonistic selection, the presence of haploid selected loci around ancestral- or novel-sex-determining regions could support their role in sex chromosome turnover.
In agreement with this hypothesis}\DIFdelend \DIFaddbegin \DIFadd{Linking haploid expression with the evolution of sex-determination}\DIFaddend , a recent transcriptome analysis in \DIFdelbegin \DIFdel{Rumex }\DIFdelend \DIFaddbegin \textit{\DIFadd{Rumex}} \DIFaddend shows that pollen-biased expression (relative to expression in flower buds or leaves) is enhanced among XY-linked genes compared to autosomal genes or compared to hemizygous genes that are only linked to the X 
...
This suggests that the spread of neo-Y chromosomes in this clade could have been favoured through linkage with haploid selected genes \DIFaddbegin \DIFadd{rather than those under sexually antagonistic selection}\DIFaddend . 
%maybe add a flag here for switching to clade-level expectations:
\DIFdelbegin \DIFdel{In general, we predict that haploid selectionincreases lability of sex-determining systems, particularly because haploid selection can cause transitions that increase or decrease sex-linkage (e. 
g., the final state of the red line in Fig \ref{fig:SexRatioBad}B is the starting state in Fig \ref{fig:SexRatioBad}A). 
}\DIFdelend \DIFaddbegin 
...
%DIFDELCMD < %%%
\DIFdel{In support of their role in sex chromosome turnover, genes expected to be under sexually-antagonistic selection }\DIFdelend \DIFaddbegin \DIFadd{To further examine this link, future theory could also include inbreeding, which is an important consideration during transitions between dioecy and hermaphroditism~\mbox{%DIFAUXCMD
\cite{Charlesworth1978}}%DIFAUXCMD
. 
...
. 
In support of this hypothesis, researchers have identified genes }\DIFaddend on recently derived sex chromosomes \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{Lindholm:2002dw,Tripathi:2009cw,Ser:2010iq}}%DIFAUXCMD
. 
Our results show, however, thattight ancestral-linkage of polymorphic loci , maintained by }\DIFdelend \DIFaddbegin \DIFadd{that might be under }\DIFaddend sexually-antagonistic \DIFdelbegin \DIFdel{selection or otherwise, can also }\DIFdelend \DIFaddbegin \DIFadd{selection~\mbox{%DIFAUXCMD
\cite{Lindholm:2002dw,Tripathi:2009cw,Roberts2009,Ser:2010iq}}%DIFAUXCMD
. 
However, we show that, if selected loci are tightly linked to the ancestral sex-determining locus, they can }\DIFaddend drive trans-GSD transitions \DIFaddbegin \DIFadd{that reduce sex-linkage (Conclusions 1 \& 2), thus widening the range of genomic locations where selection could be driving observed trans-GSD transitions}\DIFaddend .
...
\DIFaddbegin \DIFadd{This pair of conclusions apply in cases with or without haploid selection. 
}
...
\DIFdelbegin \DIFdel{Several rodent species also }\DIFdelend \DIFaddbegin \DIFadd{Furthermore, several rodent species }\DIFaddend maintain feminizing alleles along with the ancestral X and Y sex-determination alleles (reviewed in~\cite{Fredga1994}). 
...
show that }\DIFdelend \DIFaddbegin \DIFadd{It should be noted, however, }\DIFaddend that \DIFdelbegin \DIFdel{neo-Y/neo-W alleles can be favoured when they arise near to haploid and/or diploid selected loci, which also occurs in our model}\DIFdelend \DIFaddbegin \DIFadd{the dynamics of sex-determining alleles with very weak effect will be influenced by genetic drift, which itself has been shown to bias transitions towards epistatically-dominant sex-determining systems when there is no direct selection \mbox{%DIFAUXCMD
...
In particular, both can select for new sex-determining loci that are more loosely linked to loci under selection \DIFaddbegin \DIFadd{(Conclusions 2 \& 3C)}\DIFaddend . 
In addition, haploid selection can cause transitions \DIFaddbegin \DIFadd{in GSD }\DIFaddend analogous to those caused by purely sexually-antagonistic selection, eliminating the need for differences in selection between male and female diploids \DIFaddbegin \DIFadd{(Conclusion 3A, 3B \& 3C)}\DIFaddend .
...
\DIFdelbegin \DIFdel{Perhaps counterintuitively}\DIFdelend \DIFaddbegin \DIFadd{Further}\DIFaddend , transitions involving haploid selection can \DIFdelbegin \DIFdel{be driven by sex ratio selection or cause sex ratio biasesto evolve; we do not find Fisherian sex ratio selection to be an overwhelming force}\DIFdelend \DIFaddbegin \DIFadd{eliminate or generate sex-ratio biases; to leading order, selection to balance the sex ratio and the benefits of hitch-hiking with haploid selected alleles, leading to a biased sex ratio, are of equal magnitude (Conclusions 4 \& 5)}\DIFaddend . 
Overall, our results suggest several \DIFdelbegin \DIFdel{new }\DIFdelend \DIFaddbegin \DIFadd{novel }\DIFaddend scenarios under which new sex-determining systems are favoured, which could help to explain why the evolution of sex-determining systems is so dynamic. 
...
%\begin{align}
%(A)\ \ \ &\hat{p}_{Y}^{\male}=0,
%DIF < \ \hat{q}=\frac{1}{2}\left(1 - \alpha_\Delta^\male \frac{ w_{Aa}^{\male} \phi}{w_{Aa}^{\male} \phi+ w_{aa}^{\male} \psi}\right),\\
...
%DIF < (B)\ \ \ &\hat{p}_{Y}^{\male}=0,\ \hat{p}_{X}^{\female}=1,\ \hat{p}_{X}^{\male}=1, \ \hat{q}=(1 -\alpha^\male_\Delta)/2\\
%DIF < (B')\ \ \ &\hat{p}_{Y}^{\male}=1,\ \hat{p}_{X}^{\female}=0,\ \hat{p}_{X}^{\male}=0, \ \hat{q}=(1+\alpha^\male_\Delta)/2
...
%\begin{equation}
%\begin{split}
%DIF < \hat{p}^\male_X-\hat{p}^\female_X&=V_{A}\big{(}D^\male - D^\female +\alpha_{\Delta}^\male-\alpha_{\Delta}^\female \big{)}
%DIF > \hat{p}^\male_X-\hat{p}^\female_X&=\bar{p}(1-\bar{p})\big{(}D^\male - D^\female +\alpha_{\Delta}^\male-\alpha_{\Delta}^\female \big{)}
%+O(\epsilon^2)\\
%%
%DIF < \hat{p}^\male_Y-\hat{p}^\female_X&=V_{A}\left[D^\male - D^\female+\alpha_{\Delta}^\male-\alpha_{\Delta}^\female+(1-2r)(t^\male-t^\female)\right]/2r
...
%+O(\epsilon^2)\\
%%
%DIF < \hat{p}^\male_Y-\hat{p}^\male_X&=V_{A}\left(D^\male - D^\female+\alpha_{\Delta}^\male-\alpha_{\Delta}^\female+t^\male-t^\female\right)(1-2r)/2r
