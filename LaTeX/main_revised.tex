...
Predominant theories for why we observe such transitions generally conclude that novel sex-determining systems are favoured by selection if they equalise the sex ratio or increase linkage with a \textcolor{red}{sexually-antagonistic locus}. 
...
We find that associations with haploid selected loci can drive transitions between sex-determining systems \textcolor{red}{, without requiring sexually-antagonistic selection in diploids}. 
...
One novel feature of the models developed here is that we explicitly \textcolor{red}{(is this wording fair to vD\&K?)} consider loci in very tight linkage with the ancestral sex-determining locus (e.g., within the non-recombining region of a sex chromosome). % in the results section: `Tight linkage with the ancestral sex-determining locus'). 
...
\textbf{selection on loci within the non-recombining region around the sex-determining locus ($r\approx0$) drives the invasion of less closely linked sex-determining alleles ($r<R\leq1/2$) during a trans-GSD transition}.
...
\vspace{0.5cm}
\noindent\textcolor{red}{\textit{Conclusion 2A:}\
\textbf{new sex-determining alleles (causing cis- \textit{or} trans-GSD transitions) can spread if the arise in closer linkage with a locus that experiences haploid selection than the ancestral-sex-determining locus ($R<r$).} (This is different but similar to Ubeda et al (2015), who started from ESD)}
\vspace{0.5cm}
...
\noindent\textcolor{red}{\textit{Conclusion 2B:}\
...
\textcolor{red}{When $R>r$, neo-W alleles can benefit from Y-associated alleles that have higher fitness in the female diploid phase but then dissociate from these alleles during meiosis, which occurs before any subsequent haploid selection. }
...
Our model allows us to consider GSD-GSD transitions where sex-ratio bias increases, decreases, or remains equal. 
...
However, we show that loci tightly linked to the ancestral sex-determining locus can drive trans-GSD transitions that reduce linkage (Conclusion 1). 
...
% Evidence
\textcolor{red}{These results suggest a potential mechanism maintaining multiple sex determining alleles in species such as platyfish (\textit{Xiphophorus maculatus}), in which X,Y, and W alleles segregate at one locus (or two closely-linked loci) near to potentially sexually-antagonistic genes for pigmentation and sexual maturity~\cite{Kallman1965,Kallman1968, Volff2001, Schultheis2006}.
...
Nevertheless, as a proportion of all genes, a relatively small fraction are thought to be expressed and selected during competition in animal sperm, although precise estimates are uncertain~\cite{Zheng:2001fi,JOSEPH:2004haa,Vibranovski:2010et}.  
On the other hand, gametic competition may be particularly common in plants, which have a multicellular haploid stage.
Artificial selection pressures applied to male gametophytes are known to cause a response to selection (e.g.,~\cite{Hormaza:1996cv,Ravikumar:2003uo,Hedhly:2004iv,Clarke:2004ir}). 
60-70\% of all genes are expressed in the male gametophyte, and these genes exhibit stronger signatures of selection than randomly-chosen genes~\cite{Borg:2009jpa,Arunkumar:2013iq,Gossmann:2014dua}.
}
