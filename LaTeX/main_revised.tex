...
Specifically, if the selected locus ($\mathbf{A}$) is ancestrally autosomal ($r=1/2$, leading to $ \hat{p}^\male_Y-\hat{p}^\male_X =0$), a neo-W linked to the selected locus ($R<1/2$) will spread ($\lambda_{W'}^{(XY)}>1$). 
...
\noindent
In the absence of haploid selection ($t^\circ=\alpha^\circ_{\Delta}=0$), Eq \eqref{eq:lambda_neoW} indicates that trans-GSD transitions can occur if and only if the new sex-determining locus is more closely linked to a locus under selection, $R<r$, as found by \cite{vanDoorn:2010hu}. 
