...
%currently 312 words
\section*{Abstract}
Genetic sex determination is remarkably dynamic; many taxa display shifts in the location of sex-determining loci or the evolution of entirely new sex-determining systems. 
...
(2) We also consider selection upon haploid genotypes either during gametic competition (e.g., pollen/sperm competition involving multiple males) or meiosis (i.e., non-Mendelian segregation), which can cause the zygotic sex ratio to become biased. 
...
Predominant theories accounting for transitions between GSD systems by selection involve fitness differences between sexes (e.g., sexually antagonistic selection) or sex ratio selection~\cite{Blaser2012, Beukeboom:2014vb,vanDoorn2014re}. 
...
Here we use mathematical models to clarify the conditions under which new sex-determining systems spread when individuals experience selection at both diploid and haploid stages, which allows fitness differences between the sexes and sex-ratio biases to occur simultaneously. 
...
For example, there are several known cases of sex-ratio bias caused by sex-linked meiotic drive alleles (Burt and Trivers, 2006, Chapter 6~\cite{Burt:2006}) or selection among X- and Y-bearing pollen~\cite{Lloyd:1974tz,Conn:1981uw,Stehlik:2005ul,Stehlik:2006to,Field:2012fd,Field:2013cc}. 
Sex-ratio selection can thus select against the spread of linked sex-determining alleles and favour the invasion of alternative, unlinked sex-determination systems \cite{Kozielska:2010vm}.
...
We draw three main conclusions about the generic invasion of neo-Y and neo-W mutations from Table \ref{tab:haplotype_growth}:
\vspace{0.5cm}

\noindent\textit{Conclusion 1: Fisherian sex-ratio selection will favour the spread of a neo-Y and inhibit the spread of a neo-W if the ancestral zygotic sex ratio is biased towards females}, $\zeta<1/2$ (i.e., the first factor of the $\Lambda_{mi}^{(XY)}$ is greater than one for a neo-Y and less than one for a neo-W).
...
\noindent\textit{Conclusion 2: invasion by a neo-Y (neo-W) does not directly depend on the fitness of female (male) diploids}. %(for a given set of equilibrium allele frequencies).
...
\noindent\textit{Conclusion 3: invasions by a neo-Y and a neo-W are qualitatively different}.
...
(1) the $\mathbf{A}$ locus is within (or tightly linked to) the non-recombining region around the ancestral sex-determining locus ($r \approx 0$) or (2) selection is weak relative to recombination ($s^\circ$, $t^\circ$, $\alpha_{\Delta}^\circ$ of order $\epsilon<<1$). 
...
Here we assume that selection is weak ($s^\circ$, $t^\circ$, $\alpha_{\Delta}^\circ$ of order $\epsilon<<1$) and thus implicitly assume that all recombination rates ($r$, $R$ and $\rho$) are large relative to selection.
...
\noindent\textit{Conclusion 5: with intermediate recombination between the ancestral sex-determining allele and a selected locus, $0<r<1/2$, a less closely linked neo-W ($r<R$) can invade} ($\lambda_{W'}^{(XY)}>1$).
\vspace{0.5cm}
...
When $R<1/2$, the invading neo-Y becomes associated with the male meiotic drive allele and the zygotic sex ratio actually evolves to become male-biased (similar to the scenario considered in \cite{Ubeda:2015fx}).
...
\noindent\textit{Conclusion 6: when there is haploid selection, ESD can invade an ancestrally-XY system because it generates females that are either rare or have high fitness, in the same manner as a neo-W}.
\vspace{0.5cm} 
