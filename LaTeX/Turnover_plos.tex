...
The spread of a rare mutant $m$ at the \textbf{M} locus into a system with ancestral sex-determination system $i\in\{XY,ZW\}$ is determined by the leading eigenvalue ($\lambda_m^{(i)}$) of the system of eight equations describing the frequency of eggs and sperm carrying the $m$ allele in the next generation (equations \ref{eq:recursions}). %, evaluated at the above equilibrium.
...
Invasion of a rare neo-Y or neo-W allele, $m\in\{Y',W'\}$, is given by the largest solution to the quadratic $(\lambda_m^{(i)})^2+ b \lambda_m^{(i)} + c =0$. 
Here, $b= - (\Lambda_{mA}^{(i)} + \Lambda_{ma}^{(i)})+(\chi_{mA}^{(i)} + \chi_{ma}^{(i)})$ and $c = (\Lambda_{mA}^{(i)} - \chi_{mA}^{(i)}) (\Lambda_{ma}^{(i)} - \chi_{ma}^{(i)}) - \chi_{mA}^{(i)} \chi_{ma}^{(i)}$, where $\Lambda_{mj}^{(i)}$ is the multiplicative growth rate (which we will call the ``haplotypic growth rate'') of the neo-sex determination allele $m$ on background $j\in\{A,a\}$ without accounting for loss due to recombination ($R=0$), and $\chi_{mj}^{(i)}$ is the rate at which mutant haplotypes on background $j\in\{A,a\}$ recombine onto the other \textbf{A} locus background in heterozygotes (proportional to $R$), see Table \ref{tab:haplotype_growth}.
...
We are particularly concerned with the conditions under which a rare neo-sex-determining allele increases in frequency, which occurs when the largest eigenvalue is greater than one ($\lambda_m^{(i)}>1$). 
...
If both $\Lambda_{mA}^{(i)}$ and $\Lambda_{ma}^{(i)}$ are greater than one, then the leading eigenvalue will always be greater than one, regardless of the linkage between the neo-sex determination factor and the selected locus ($R$), see \nameref{app:inv_cond} for details.
...
Interpreting this condition, if we assume that only the $mA$ haplotype has a positive growth rate when $R=0$, $\Lambda_{ma}^{(i)}<1<\Lambda_{mA}^{(i)}$, the first term on the left-hand side of \eqref{eq:lambdasGen} is negative and invasion requires that the $mA$ haplotypic growth rate ($\Lambda_{mA}^{(i)} - 1$) and the rate at which $mA$ haplotypes are produced by recombination in $ma$ haplotypes ($\chi_{ma}^{(i)}$) are sufficiently large relative to the $ma$ haplotype rate of decline ($1-\Lambda_{ma}^{(i)}$) and the rate of loss of $mA$ haplotypes due to recombination ($\chi_{mA}^{(i)}$).
...
For example, when $AA$ individuals have low fitness in females yet the $A$ is polymorphic or fixed on the X due to overdominance in males (Fig \ref{fig:regionplots}B and \ref{fig:regionplots}C), both neo-W-$A$ and neo-W-$a$ haplotypes produce fewer unfit $AA$ females.
...
Assuming selection is weak relative to recombination, van Doorn and Kirkpatrick~\cite{vanDoorn:2010hu} showed that invasion by a neo-W occurs under the same conditions as `pseudo-fixation' (at pseudo-fixation the neo-W reaches its maximum frequency among eggs, which is usually 1/2). 
An equivalent analysis is not possible where recombination rates are low. 
However, numerical simulations demonstrate that the neo-SDR does not necessarily reach pseudo-fixation, which can lead to the stable maintenance of a mixed sex-determining system, in which X, Y, Z, and W alleles all segregate (e.g., \nameref{fig:freqAll}B,C). 
...
This can allow neo-W invasion ($\lambda_{W'}^{(XY)}>1$), even when the neo-sex-determining region is less closely linked to the selected locus ($R>r$). 
...
Whenever $R<1/2$, the neo-Y becomes associated with the male meiotic drive allele such that the zygotic sex ratio actually evolves to become biased towards males.
...
For example in anancestral-ZW system with the same selection regime (haploid selection in males), a neo-Y can spread despite the fact that the zygotic sex is initially even; in this case, the neo-Y spreads because it is often found in males and can, if it carries the driven allele $a$, benefit from haploid selection in males (Fig \ref{fig:SexRatioBad}B). 
...
However, because a neo-Y (neo-W) is always in males (females) and is epistatically dominant to the ancestral sex-determining locus, the characteristic polynomial factors into two quadratics times $(\lambda_m^{(i)})^4$.  
One quadratic governs the change in the frequency of the new sex determination factor associated with the A and a allele frequencies, summing across backgrounds at the old sex determination factor (i.e., summing haplotypes carrying the original X and Y). 
...
We therefore focus on the leading eigenvalue based on the root to the first quadratic $f(\lambda_m^{(i)})=(\lambda_m^{(i)})^2+ b \lambda_m^{(i)} + c =0$, as described in the text.  
Since $f(\Lambda_{mA}^{(i)})$ and $f(\Lambda_{ma}^{(i)})$ are of opposite signs, the leading eigenvalue must fall between these two quantities and is the larger of them when $R = 0$ (the Perron-Frobenius theorem guarantees that the largest eigenvalue is positive, unique, and real).  
Thus, $\lambda_m^{(i)}>1$ if both $\Lambda_{mA}^{(i)}>1$ and $\Lambda_{ma}^{(i)}>1$; similarly, $\lambda_m^{(i)}<1$ if both $\Lambda_{mA}^{(i)}<1$ and $\Lambda_{ma}^{(i)}<1$.
If only one haplotypic growth rate is greater than one ($\Lambda_{ma}^{(i)}<1<\Lambda_{mA}^{(i)}$ or $\Lambda_{mA}^{(i)}<1<\Lambda_{ma}^{(i)}$), then $\lambda_m^{(i)}$ is greater than one when condition \eqref{eq:lambdasGen} is met. 
Thus, the invasion of a neo-sex-determining allele is determined by the haplotypic growth rates without accounting for loss due to recombination ($\Lambda_{mj}^{(i)}$ terms) and the rates at which recombination between the \textbf{M} and \textbf{A} loci breaks apart these haplotypes ($\chi_{mj}^{(i)}$).
