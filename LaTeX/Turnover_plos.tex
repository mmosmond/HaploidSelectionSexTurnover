...
These parameters are marked by daggers in Fig \ref{fig:regionplots}B and C, which show that neo-W invasion is expected for any $R$ ($\lambda_{WA},\lambda_{Wa}>1$) if the $a$ allele is nearly fixed on the Y (black lines in this figure; not stable for $r>>0$). 
...
By contrast, $\lambda_{WA}$ and $\lambda_{Wa}$ tend to be reduced when the $A$ allele is favoured during male gametic competition, panels D-F.
