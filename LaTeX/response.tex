...
\noindent\paragraph{Response}
\textcolor{blue}{We have made the suggested replacements. There is one exception. In the abstract we use the familiar term ``sperm competition" as an example of gametic competition, but we now explicitly state that we imply sperm from multiple males.
...
The fifth paragraph discusses the potential complications of haploid selection for transitions (another hole in current theory).
...
These are highlighted again in the results, in the style of Orr 1998:\\
%(1) transitions are determined by both sex-ratio selection and selection on associated alleles,\\
%(2) cis- and trans-GSD transitions are qualitatively different,\\
...
(2) when selection is weak relative to recombination, sex-ratio selection is exactly equal in magnitude to the selection imposed by associations with selected alleles, and\\
(3) transitions from genetic to environmental sex-determination are not straightforwardly predicted by selection to balance the zygotic sex ratio when haploid selection is present.
\vspace{0.25cm}\\
...
\noindent\paragraph{Response}
\textcolor{blue}{We have rewritten the main conclusions (see Response 2.2).}
